...
new = '''  const [session, setSession] = useState<{ mode: "lesson" | "quick"; questions: LearnQuestion[]; index: number; selected?: number; correct: number; phase?: "intro" | "discover" | "apply" | "quiz" | "finish"; reveal?: boolean; choice?: number } | null>(null);'''
assert old in s, 'session model missing'
s = s.replace(old, new)

old = '''  function closeLesson() {
...
assert old in s, 'closeLesson missing'
s = s.replace(old, new)

old = '''  function startLesson(lessonId = selected.id) {
...
new = '''  function buildLessonQuestions(lesson: (typeof ALL_LESSONS)[number]) {
...
assert old in s, 'startLesson missing'
s = s.replace(old, new)

s = s.replace('setSession({ mode: "quick", questions: pool.slice(0, 5), index: 0, correct: 0 });', 'setSession({ mode: "quick", questions: pool.slice(0, 5), index: 0, correct: 0, phase: "quiz" });')

old = '''  function continueSession() {
    if (!session) return;
    if (session.index === -1) {
...
assert old in s, 'continueSession missing'
s = s.replace(old, new)

start = s.index('            {session.index === -1 ? (')
end = s.index('          </div>\n        </div>\n      ) : null}', start)
body = '''            {session.mode === "lesson" && session.phase === "intro" ? (
