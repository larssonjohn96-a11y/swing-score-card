...
import { COURSE_DISTANCES, holeStars, maxStars } from "@/lib/speed-course";
import { SpeedStars as Stars } from "./speed-stars";
const positions = [
  [16, 27],
  [48, 25],
  [83, 27],
...
export function SpeedCourseMap({
  holes,
  cursor,
  reference,
}: {
  holes: import("@/lib/speed-course").CourseShot[][];
  reference: number;
  ...
  return (
    ...
    >
      ...
      {COURSE_DISTANCES.map((d, i) => (
        <div
          key={i}
          className="speed-map-hole absolute flex -translate-x-1/2 -translate-y-1/2 flex-col items-center gap-0.5"
          style={{ left: `${positions[i][0]}%`, top: `${positions[i][1]}%` }}
        >
          <span
            aria-label={`Slag ${i + 1}, $Speedeter`}
          ...
          <Stars
            count={holeStars(holes[i] ?? [], i, reference)}
            max={maxStars(i)}
            zero={holes[i]?.length === 1 && holeStars(holes[i], i, reference) === 0}
          />
        </div>
      ))}
      ...
      {point && (
        <div
          className="speed-map-golfer pointer-events-none absolute z-10 -translate-x-1/2 -translate-y-full transition-[left,top] duration-700 ease-in-out motion-reduce:transition-none"
          style={{
          ...
          aria-label={
            cursor === "halfway"
              ? "Du är vid Halfway House"
              : `Du är vid slag ${Number(cursor) + 1}`
          }
        >
          <svg
            role="img"
