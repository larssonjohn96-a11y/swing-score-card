import { STAR_STEPS, starLevel } from "@/lib/speed-course";
import { Star } from "lucide-react";
export function SpeedStars({
  count = 0,
  max = 3,
  large = false,
  zero = false,
  ...
  return (
    ...
    >
      {Array.from({ length: max }, (_, i) => (
        <span
          key={i}
          className={`relative inline-block shrink-0 ${large ? "h-12 w-12" : "h-4 w-4"} ${i < count ? "speed-star" : ""}`}
          style={{
...
export function SpeedMilestones({ stars, previous }: { stars: number; previous?: number }) {
    ...
    <section className="rounded-3xl border border-amber-100 bg-white p-4">
      ...
      >
        {STAR_STEPS.map((n, i) => (
          <div
            key={n}
            className={`flex flex-1 items-center justify-center rounded-t-xl text-sm font-black ${previous !== undefined && previous < n && stars >= n ? "speed-level-up" : ""} ${stars >= n ? "bg-amber-400 text-amber-950" : "bg-slate-100 text-slate-400"}`}
