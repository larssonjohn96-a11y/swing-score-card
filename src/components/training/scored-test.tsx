...
export type ScoredTestProps = {
  ...
  clubGroups?:ClubGroup[]; liquidGlass?:boolean; hitMissColors?:boolean; multiplayer?:boolean;
...
export function ScoredTest(props:ScoredTestProps){
  ...
  function start(){
    ...
    setShots([]);setFriendShots([]);setCurrentPlayer("self");setPhase("test");
  }
  function register(value:number){
    if(withFriend&&selectedFriend){
      ...
      if(nextFriend.length>=total){
        ...
        setPhase("result");
      }else setCurrentPlayer("self");
      return;
    }
    const next=[...shots,value];setShots(next);
    if(next.length>=total){saveSession(props.testId,next,variant,club);setPhase("result")}
  }
  function undo(){
    if(withFriend&&selectedFriend){
      if(currentPlayer==="friend"&&shots.length>friendShots.length){setShots(s=>s.slice(0,-1));setCurrentPlayer("self");return;}
      if(currentPlayer==="self"&&friendShots.length){setFriendShots(s=>s.slice(0,-1));setCurrentPlayer("friend");return;}
    }
  ...
  if(phase==="intro")return <main style={LIGHT_SURFACE} className="mx-auto flex min-h-[100dvh] w-full max-w-md flex-col bg-background px-5 pb-5 pt-4 text-foreground">
    ...
    <section className={`mt-5 rounded-[28px] border p-5 ${glass}`}><p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-muted-foreground">{props.eyebrow}</p><h1 className="mt-2 font-display text-4xl leading-none">{props.title}</h1><p className="mt-3 text-[13px] leading-relaxed text-muted-foreground">{props.intro}</p></section>

    {props.multiplayer?<div className="mt-4"><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-muted-foreground">Spelläge</p><div className="mt-2 grid grid-cols-2 gap-2"><button type="button" onClick={()=>{setWithFriend(false);setSelectedFriend(null)}} className={`rounded-3xl border p-4 text-left backdrop-blur-xl transition-all ${!withFriend?"border-blue-500/45 bg-blue-500/12 shadow-sm":"border-white/60 bg-white/60"}`}><User className="h-4 w-4 text-blue-600"/><span className="mt-2 block font-display text-xl">Solo</span></button><button type="button" onClick={()=>setWithFriend(true)} className={`rounded-3xl border p-4 text-left backdrop-blur-xl transition-all ${withFriend?"border-blue-500/45 bg-blue-500/12 shadow-sm":"border-white/60 bg-white/60"}`}><Users className="h-4 w-4 text-blue-600"/><span className="mt-2 block font-display text-xl">Med vän</span></button></div>{withFriend?<div className={`mt-2 rounded-3xl border p-3 ${glass}`}><p className="text-[9px] font-bold uppercase tracking-[0.16em] text-muted-foreground">Välj vän</p>{friends.length?<div className="mt-2 flex flex-wrap gap-2">{friends.map(friend=><button key={friend.id} type="button" onClick={()=>setSelectedFriend(friend)} className={`rounded-full border px-3 py-2 text-xs font-semibold transition-all ${selectedFriend?.id===friend.id?"border-blue-500 bg-blue-500/15 text-blue-700":"border-border/70 bg-white/55"}`}>{friend.other.displayName}</button>)}</div>:<p className="mt-2 text-xs text-muted-foreground">Inga accepterade vänner ännu. Lägg till en vän under Vänner först.</p>}</div>:null}</div>:null}
    ...
    {props.clubGroups?<div className="mt-4"><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-muted-foreground">Välj klubbgrupp</p><div className="mt-2 grid grid-cols-2 gap-2">{props.clubGroups.map(group=>{const selected=club===group.label;return <button key={group.label} type="button" onClick={()=>setClub(group.label)} className={`rounded-3xl border p-4 text-left backdrop-blur-xl transition-all active:scale-[0.98] ${selected?"border-foreground/55 bg-foreground text-background shadow-[0_16px_34px_-24px_rgba(15,23,42,0.7)]":"border-white/60 bg-white/60 text-foreground shadow-[0_12px_30px_-26px_rgba(15,23,42,0.45)]"}`}><span className="block font-display text-xl leading-none">{group.label}</span><span className={`mt-1.5 block text-[11px] font-semibold ${selected?"text-background/65":"text-muted-foreground"}`}>{clubGroupDetail(group)}</span></button>})}</div></div>:null}
  ...
  if(phase==="test"){
    ...
    const prompt=prompts[index],running=shots.reduce((a,b)=>a+b,0),friendRunning=friendShots.reduce((a,b)=>a+b,0);
    ...
    return <main style={LIGHT_SURFACE} className="mx-auto flex min-h-[100dvh] w-full max-w-md flex-col bg-background px-5 pb-6 text-foreground">
      ...
      {withFriend?<div className="mt-3 grid grid-cols-2 gap-2"><div className={`rounded-2xl border px-3 py-2.5 ${currentPlayer==="self"?"border-blue-500/45 bg-blue-500/12":"border-white/60 bg-white/55"}`}><p className="text-[9px] font-bold uppercase tracking-[0.14em] text-muted-foreground">Du</p><p className="font-display text-xl">{running}</p></div><div className={`rounded-2xl border px-3 py-2.5 text-right ${currentPlayer==="friend"?"border-blue-500/45 bg-blue-500/12":"border-white/60 bg-white/55"}`}><p className="text-[9px] font-bold uppercase tracking-[0.14em] text-muted-foreground">{selectedFriend?.other.displayName}</p><p className="font-display text-xl">{friendRunning}</p></div></div>:null}
      ...
      <div className={`mt-4 grid gap-3 ${props.optionCols===3?"grid-cols-3":"grid-cols-2"}`}>{props.options.map(option=>{const hit=props.hitMissColors&&option.value>0;const miss=props.hitMissColors&&option.value===0;const tone=hit?"border-emerald-500/25 bg-emerald-500/[0.08] text-emerald-700 active:border-emerald-600 active:bg-emerald-500/30":miss?"border-red-500/25 bg-red-500/[0.08] text-red-600 active:border-red-600 active:bg-red-500/30":"border-border bg-card text-primary";return <button key={option.label} onClick={()=>register(option.value)} className={`flex h-[104px] flex-col items-center justify-center rounded-3xl border px-1 text-center shadow-[0_14px_28px_-24px_rgba(15,23,42,0.5)] backdrop-blur-xl transition-all active:scale-[0.97] ${tone}`}><span className="font-display text-2xl leading-none">{option.label}</span>{option.hint?<span className="mt-2 text-[11px] font-semibold opacity-70">{option.hint}</span>:null}</button>})}</div>
      {!withFriend?<div className={`mt-3 flex items-center justify-between rounded-2xl border px-4 py-3 text-sm ${glass}`}><span className="text-muted-foreground">{props.runningLabel??"Hittills"}</span><span className="font-semibold tabular-nums">{running}</span></div>:null}{(shots.length>0||friendShots.length>0)?<button onClick={undo} className="mt-3 self-center text-xs font-semibold text-muted-foreground">↶ Ändra föregående slag</button>:null}
    </main>;
  }

  const analysis=props.analyze(shots,prompts,variant);const friendAnalysis=withFriend?props.analyze(friendShots,prompts,variant):null;const category=categoryForTest(props.selfTo);
  return <main style={LIGHT_SURFACE} className="mx-auto min-h-screen w-full max-w-md bg-background px-5 pb-16 pt-6 text-foreground">
    ...
    {withFriend&&friendAnalysis?<section className="mt-5 grid grid-cols-2 gap-3"><div className="rounded-3xl border border-blue-500/25 bg-blue-500/[0.08] p-5 text-center shadow-[0_18px_44px_-34px_rgba(37,99,235,0.6)] backdrop-blur-xl"><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-blue-600">Du</p><p className="mt-2 font-display text-5xl text-blue-600">{analysis.headline.value}</p></div><div className="rounded-3xl border border-blue-500/25 bg-blue-500/[0.08] p-5 text-center shadow-[0_18px_44px_-34px_rgba(37,99,235,0.6)] backdrop-blur-xl"><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-blue-600">{selectedFriend?.other.displayName}</p><p className="mt-2 font-display text-5xl text-blue-600">{friendAnalysis.headline.value}</p></div></section>:<section className="mt-5 rounded-3xl border border-white/65 bg-gradient-to-br from-white/78 via-white/62 to-blue-500/[0.06] p-6 text-center shadow-[0_22px_52px_-36px_rgba(15,23,42,0.55)] backdrop-blur-2xl"><p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-muted-foreground">{analysis.headline.label}</p><p className="mt-2 font-display text-6xl leading-none text-primary">{analysis.headline.value}</p>{analysis.headline.hint?<p className="mt-2 text-xs text-muted-foreground">{analysis.headline.hint}</p>:null}</section>}
    {analysis.metrics.length?<section className="mt-3 grid grid-cols-2 gap-3">{analysis.metrics.map(m=><div key={m.label} className="rounded-3xl border border-white/65 bg-white/60 p-4 shadow-[0_18px_44px_-34px_rgba(15,23,42,0.5)] backdrop-blur-2xl"><p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">{m.label}</p><p className="mt-1.5 font-display text-2xl leading-none">{m.value}</p>{m.hint?<p className="mt-1 text-[11px] text-muted-foreground">{m.hint}</p>:null}</div>)}</section>:null}
...
export function AnalysisSections({sections,liquidGlass=false}:{sections:Analysis["sections"];liquidGlass?:boolean}):ReactNode{return <>{sections.map(section=><section key={section.title} className={`mt-3 rounded-3xl border p-4 ${liquidGlass?"border-white/65 bg-white/58 shadow-[0_18px_44px_-34px_rgba(15,23,42,0.5)] backdrop-blur-2xl":"border-border bg-card"}`}><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-muted-foreground">{section.title}</p><div className="mt-3 space-y-2.5">{section.rows.map(row=><div key={row.label}><div className="flex items-center justify-between text-[13px]"><span className="text-muted-foreground">{row.label}</span><span className="font-semibold tabular-nums">{row.value}</span></div>{typeof row.ratio==="number"?<div className="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-white/70"><div className="h-full rounded-full bg-primary" style={{width:`${Math.max(0,Math.min(1,row.ratio))*100}%`}}/></div>:null}</div>)}</div>{section.note?<p className="mt-3 text-[11px] text-muted-foreground">{section.note}</p>:null}</section>)}</>}
