import { STAR_STEPS, starLevel } from "@/lib/approach-course";
import { Star } from "lucide-react";
export function ApproachStars({
  count = 0,
  max = 3,
  large = false,
  zero = false,
    ...
    >
      {Array.from({ length: max }, (_, i) => (
        <Star
          key={i}
          aria-hidden
          className={`${large ? "h-12 w-12" : "h-4 w-4"} ${i < count ? "approach-star fill-amber-400 text-amber-500" : zero ? "fill-slate-500 text-slate-600" : "fill-white/70 text-slate-300"}`}
          style={
            i < count
              ? {
...
export function ApproachMilestones({ stars, previous }: { stars: number; previous?: number }) {
    ...
    <section className="rounded-3xl border border-amber-100 bg-white p-4">
      ...
      >
        {STAR_STEPS.map((n, i) => (
          <div
            key={n}
            className={`flex flex-1 items-center justify-center rounded-t-xl text-sm font-black ${previous !== undefined && previous < n && stars >= n ? "approach-level-up" : ""} ${stars >= n ? "bg-amber-400 text-amber-950" : "bg-slate-100 text-slate-400"}`}
