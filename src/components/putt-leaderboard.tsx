...
import { puttAverage } from "@/lib/putt-course";
import { fetchFriendSnapshot, listFriendships, pushPlayerSnapshot } from "@/lib/friends-cloud";
import type { CourseRound } from "@/lib/putt-course";
type Row = ReturnType<typeof puttAverage> & { id: string; name: string };
export function PuttLeaderboard({
  userId,
  history,
  playerName,
}: {
        ...
        const friends = await Promise.all(
          accepted.map(async (f) => {
            ...
            return {
              ...
              count: value("putt-round-count"),
              points: value("putt-round-stars"),
              stars: value("putt-round-stars"),
            };
          }),
        );
        if (alive) {
          ...
          return (
            ...
            >
              ...
              <div className="text-right">
                ...
                <p className="text-xs text-slate-500">{r.count ? `av 16 ★` : ""}</p>
              </div>
            </li>
          );
        })}
