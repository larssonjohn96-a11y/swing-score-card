...
import { driverAverage } from "@/lib/driver-course";
import { fetchFriendSnapshot, listFriendships, pushPlayerSnapshot } from "@/lib/friends-cloud";
import type { CourseRound } from "@/lib/driver-course";
type Row = ReturnType<typeof driverAverage> & { id: string; name: string };
        ...
        const friends = await Promise.all(
          accepted.map(async (f) => {
            ...
            return {
              ...
              count: value("driver-round-count"),
              points: value("driver-round-points"),
              stars: value("driver-round-stars"),
            };
          }),
        );
        if (alive) {
