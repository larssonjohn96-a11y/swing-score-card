...
import { buildActivityReview, type ActivityReviewInput, type ActivityCategory } from "@/lib/activity-review";
...
export function ActivityReview({
  ...
  return (
    <ActivityReviewShell
      ...
      positive={review.counts[0].count ? `${review.counts[0].count} exceptionella slag` : review.good ? `${review.good} bra resultat` : null}
      negative={review.counts[5].count ? `${review.counts[5].count} stora tapp` : review.poor ? `${review.poor} att förbättra` : null}
      activity={activity}
    ...
    >
      ...
      <section className="space-y-1.5">
        ...
        <p className="pb-1 text-xs text-slate-500">Momentets resultat enligt SG4:s bedömningsregler. Tryck för att filtrera.</p>
        ...
        {review.outcomes.some(row => !row.category) && <p className="text-xs text-slate-500">{review.outcomes.filter(row => !row.category).length} försök saknar tillräckligt underlag för kategorisering.</p>}
      </section>
      {review.best && (
        <section className="rounded-2xl border border-teal-100 bg-teal-50 p-4">
          ...
          <p className="mt-1 text-sm font-bold">{review.best.label} · {review.best.result}</p>
          {review.best.context && <p className="mt-1 text-xs text-slate-600">{review.best.context}</p>}
          <p className="mt-1 text-xs text-teal-700">{review.best.category}. {review.best.basis}</p>
        </section>
      )}
      {review.worst && (
        <section className="rounded-2xl border border-orange-100 bg-orange-50 p-4">
          ...
          <p className="mt-1 text-sm font-bold">{review.worst.label} · {review.worst.result}</p>
          {review.worst.context && <p className="mt-1 text-xs text-slate-600">{review.worst.context}</p>}
          <p className="mt-1 text-xs text-orange-700">{review.worst.category}. {review.worst.basis}</p>
        </section>
      )}
      <section className="space-y-2">
        <h3 className="font-bold">Försök för försök</h3>
        {filter !== "all" && <button type="button" onClick={() => setFilter("all")} className="text-sm text-blue-700">Visa alla försök</button>}
        {rows.map((row, index) => (
          <div key={`${row.label}-${index}`} className="rounded-xl border border-slate-200 p-3">
            <p className="text-sm font-bold">{row.label}</p>
            {row.category && <span className={`mt-1 inline-block rounded-lg px-2 py-1 text-xs ${categoryStyle[row.category]}`}>{row.category}</span>}
      ...
      <details className="rounded-xl bg-slate-50 p-3 text-xs text-slate-500">
        ...
        <p className="mt-2">
          ...
          Kategorierna följer preliminära regler för resultatzon, relativ avvikelse eller träff/miss.
          Förväntat är en regelkategori, inte en verifierad HCP-20-jämförelse.
          Bästa slag och största tapp rangordnas med samma regel inom passet; tapp anges inte i förlorade slag.
          Reglerna kan kalibreras utan att ändra registrerade resultat. Modellversion {review.version}.
        </p>
      </details>
    </ActivityReviewShell>
  );
