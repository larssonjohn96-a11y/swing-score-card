...
import { roundStars, type CourseRound } from "@/lib/approach-course";
export function ApproachProgress({ history }: { history: CourseRound[] }) {
  ...
  const graph = (interactive: boolean) => (
    <svg
      viewBox="0 0 310 185"
      role="img"
      aria-label="Stjärnor per hel runda, från 0 till 18"
      className="w-full"
    >
      {[0, 6, 12, 18].map((v) => (
        <g key={v}>
          <line
            ...
            stroke="#e2e8f0"
          />
          <text x="2" y={159 - v * (128 / 18)} fontSize="10" fill="#64748b">
            {v}
          </text>
        </g>
      ))}
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="!inset-0 !h-[100dvh] !w-full !max-w-none !translate-x-0 !translate-y-0 !rounded-none bg-white text-slate-950">
          <div className="m-auto w-full max-w-lg">
            ...
            <DialogDescription className="my-3">Stjärnor per hel runda · max 18</DialogDescription>
            {graph(true)}
            <p className="mt-4 text-center font-bold text-blue-700">
              {selected !== null && rounds[selected]
                ? `${new Date(rounds[selected].finishedAt).toLocaleDateString("sv-SE")} · ${values[selected]} av 18 ★`
                : values.length
