...
export function SpeedCourseAnalysis({ round }: { round: CourseRound }) {
  ...
  const rows = round.holes.flatMap((h, i) =>
    h.length
      ? [
          {
            hole: i + 1,
                ...
                : shotPoints(h[0]) >= 75
                  ...
                  : shotPoints(h[0]) >= 60
                    ...
                    : shotPoints(h[0]) >= 40
                      ...
                      : shotPoints(h[0]) >= 20
                        ? "Svagt"
                        : "Stort tapp") as ActivityCategory,
            },
          },
        ]
      : [],
  ...
  const review = buildActivityReview({
    title: "Rundanalys",
    handicap: hcp,
    modelId: "speed-course-v1",
    outcomes: rows.map((r) => ({
      label: `Slag ${r.hole}`,
      context: "Bollhastighet",
      result: `${shotPoints(r.shot)} speedpoäng`,
      rank: -shotPoints(r.shot),
    ...
    >
      ...
      <DialogContent className="!animate-none !fixed !inset-0 !left-0 !top-0 !h-[100dvh] !max-h-none !w-full !max-w-none !translate-x-0 !translate-y-0 !rounded-none !border-0 !p-0 !gap-0 overflow-hidden !bg-transparent text-slate-950 [&>button]:z-30 [&>button]:bg-white [&>button]:p-2">
        ...
        <DialogDescription className="sr-only">
          Ditt estimerade drives-handicap och slag för slag, grupperat per slag.
        </DialogDescription>
        {canViewDetailedBreakdowns ? (
          <>
            ...
            >
              <div className="max-h-[85dvh] w-full max-w-md space-y-4 overflow-y-auto rounded-[26px] border border-slate-200 bg-white p-5 shadow-xl">
                <div>
                  <h2 className="text-2xl font-black">Estimerat speed-HCP</h2>
                ...
                <div className="grid grid-cols-2 gap-3 rounded-2xl bg-slate-50 p-3 text-center">
                  <div>
                    <strong className="text-2xl text-blue-700">{roundPoints(round)}</strong>
                    <p className="text-xs text-slate-500">Speedpoäng · max 100</p>
                  </div>
                  <div>
                ...
                <div ref={groups} className="scroll-mt-5 space-y-3">
                  <div className="flex items-center justify-between">
                    <h3 className="font-black">{filter ?? "Slag för slag"}</h3>
                    {filter && (
                      <button
                  ...
                  {rows
                    .filter((r) => !filter || r.category.category === filter)
                    .map((r) => (
                      <section
                        key={r.hole}
                        className="overflow-hidden rounded-2xl border border-slate-200"
                        aria-label={`Slag ${r.hole}`}
                      >
                        <h4 className="bg-blue-50 p-3 font-black">Slag {r.hole}</h4>
                        <div className="space-y-2 p-3 text-sm">
                          <p className="font-bold">{formatSpeedResult(r.shot)}</p>
                          <div className="flex items-center justify-between gap-2">
                            <span
                              className={`rounded-full px-2 py-1 text-xs font-bold ${r.category.category === "Exceptionellt" ? "bg-teal-100 text-teal-800" : r.category.category === "Utmärkt" ? "bg-blue-100 text-blue-800" : r.category.category === "Bra" ? "bg-emerald-100 text-emerald-800" : r.category.category === "Förväntat" ? "bg-slate-100 text-slate-700" : r.category.category === "Svagt" ? "bg-orange-100 text-orange-800" : "bg-rose-100 text-rose-800"}`}
                            >
                              {r.category.category}
                            </span>
                            <strong className="text-blue-700">
                              {shotPoints(r.shot)} speedpoäng
                            </strong>
                          </div>
                        </div>
                      </section>
          ...
          <div className="mx-auto flex h-full max-w-md flex-col justify-center p-6 text-center">
            ...
            <p className="mt-2 text-slate-500">
              Se ditt estimerade handicap och analysen för varje slag med SG4+.
            </p>
