...
import { roundPoints, type CourseRound } from "@/lib/speed-course";
export function SpeedProgress({ history }: { history: CourseRound[] }) {
  ...
  const graph = (interactive: boolean) => (
    <svg
      viewBox="0 0 310 185"
      role="img"
      aria-label="Speedpoäng per hel runda, från 0 till 100"
      className="w-full"
    >
        ...
        <g
          ...
          {...(interactive
            ? {
                role: "button",
                tabIndex: 0,
                "aria-label": `Runda ${i + 1}: ${values[i]} speedpoäng`,
        ...
        <DialogContent className="!inset-0 !h-[100dvh] !w-full !max-w-none !translate-x-0 !translate-y-0 !rounded-none bg-white text-slate-950">
          <div className="m-auto w-full max-w-lg">
            ...
            <DialogDescription className="my-3">
              Speedpoäng per hel runda · max 100
            </DialogDescription>
            {graph(true)}
