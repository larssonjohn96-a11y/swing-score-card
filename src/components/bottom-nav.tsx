...
import { Bot, BriefcaseBusiness, ChevronRight, Flag, Gauge, Home, ListChecks, Menu, Plus, Swords, Target, Trophy, Users } from "lucide-react";
...
function playToneClasses(tone: string) {
  if (tone === "friend") return { card: "border-red-300/30 bg-gradient-to-r from-blue-500/[.06] via-white/[.62] to-red-500/[.09]", icon: "border border-white/40 bg-slate-950/90 text-white" };
  if (tone === "bot") return { card: "border-red-300/30 bg-gradient-to-r from-red-500/[.10] via-white/[.58] to-red-400/[.05]", icon: "border border-white/40 bg-red-600/90 text-white" };
  if (tone === "team") return { card: "border-red-300/30 bg-gradient-to-r from-red-500/[.07] via-white/[.60] to-violet-500/[.08]", icon: "border border-white/40 bg-violet-600/90 text-white" };
  return { card: "border-red-300/25 bg-white/[.58]", icon: "border border-white/40 bg-red-500/15 text-red-600" };
}
  ...
  return <>
    <nav className="fixed left-1/2 z-40 w-[calc(100%-24px)] max-w-md -translate-x-1/2 overflow-visible rounded-[30px] border border-white/75 bg-card/66 shadow-[0_18px_48px_-20px_rgba(15,23,42,.52),inset_0_1px_0_rgba(255,255,255,.95)] backdrop-blur-[28px] supports-[backdrop-filter]:bg-card/56" style={{ bottom: "max(10px, env(safe-area-inset-bottom))" }}>
      <div className="mx-auto grid h-[68px] w-full grid-cols-[1fr_1fr_74px_1fr_1fr] items-center gap-0.5 px-2.5 py-1.5">
        ...
        <button type="button" onClick={() => setPlayOpen(true)} aria-label="Spela" className="relative -mt-5 flex h-[66px] w-[66px] items-center justify-center justify-self-center rounded-full border border-red-100/55 bg-[radial-gradient(circle_at_32%_18%,rgba(255,255,255,.62),transparent_28%),linear-gradient(145deg,rgba(248,113,113,.94),rgba(185,28,28,.88))] text-white shadow-[inset_0_1px_0_rgba(255,255,255,.66),inset_0_-8px_18px_rgba(127,29,29,.22),0_16px_30px_-12px_rgba(185,28,28,.72),0_0_0_6px_rgba(255,255,255,.24)] backdrop-blur-[28px] transition duration-200 active:scale-[.94]">
          <span className="absolute inset-[5px] rounded-full border border-white/20 bg-white/[.06]" />
          <span className="absolute left-[16px] right-[16px] top-[10px] h-px rounded-full bg-white/70" />
    ...
    <Sheet open={playOpen} onOpenChange={setPlayOpen}>
      <SheetContent side="bottom" className="rounded-t-[34px] border-white/70 bg-card/80 px-5 pb-7 pt-6 shadow-[0_-24px_70px_-30px_rgba(15,23,42,.45)] backdrop-blur-[30px]">
        <SheetHeader className="space-y-1.5"><SheetTitle className="text-left text-[30px] leading-none">Spela</SheetTitle><p className="text-left text-sm text-muted-foreground">Välj hur du vill tävla.</p></SheetHeader>
