 ...
 * samtidigt (t.ex. "alla golfare" + en åldersgrupp) i EN graf istället för
 * flera separata kort – samma lösning som löste layoutbuggen i Speed-
 * testets bellcurve (SpeedComparisonBellCurve), generaliserad för HCP-
 * semantik (LÄGRE är bättre, till skillnad från ball speed).
 *
 * Bäst hamnar till höger, samma konvention som Approach-testets
 * HcpBellCurve.
 */
  ...
  return (
    <div className="rounded-3xl border border-border bg-card p-5">
      ...
      >
        <defs>
          {groups.map((g) => (
            <linearGradient key={g.label} id={`hcp-grad-${g.label}`} x1="0" y1="0" x2="0" y2="1">
        ...
        {groups.map((g) => {
          ...
          return (
            <g key={g.label}>
              <path d={area} fill={`url(#hcp-grad-${g.label})`} />
              <path
                d={line}
                fill="none"
                stroke={g.color}
