...
export function BunkerAnalysis({ round }: { round: CourseRound }) {
  ...
  const rows = round.holes.flatMap((shots, i) =>
    shots.map((points, j) => ({
      group: i + 1,
      ball: j + 1,
      points,
      zone: BUNKER_ZONES.find((z) => z.points === points)!,
    })),
  );
  const review = buildActivityReview({
    title: "Rundanalys",
    handicap: hcp,
    modelId: "bunker-course-zones-v1",
    outcomes: rows.map((r) => ({
      label: `Omgång ${r.group} · boll ${r.ball}`,
      context: "Bunker",
      result: r.zone.label,
      rank: 4 - r.points,
      category: r.zone.category as ActivityCategory,
    })),
  });
  useEffect(() => {
    ...
    >
      ...
      <DialogContent className="!animate-none !fixed !inset-0 !left-0 !top-0 !h-[100dvh] !max-h-none !w-full !max-w-none !translate-x-0 !translate-y-0 !rounded-none !border-0 !p-0 !gap-0 overflow-hidden !bg-transparent text-slate-950 [&>button]:z-30 [&>button]:bg-white [&>button]:p-2">
        ...
        <DialogDescription className="sr-only">
          Ditt estimerade bunker-handicap och slag för slag, grupperat per omgång.
        </DialogDescription>
        {canViewDetailedBreakdowns ? (
          <>
            ...
            >
              <div className="max-h-[85dvh] w-full max-w-md space-y-4 overflow-y-auto rounded-[26px] border border-slate-200 bg-white p-5 shadow-xl">
                <div>
                  <h2 className="text-2xl font-black">Estimerat bunker-HCP</h2>
                ...
                <div className="grid grid-cols-2 gap-3 rounded-2xl bg-slate-50 p-3 text-center">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">Totalpoäng</p>
                  </div>
                  <div>
                    <strong className="text-2xl text-amber-600">
                      {roundStars(round).toFixed(1).replace(".", ",")} ★
                    </strong>
                ...
                <div className="space-y-1.5" aria-label="Filtrera slag">
                  {review.counts
                    .filter((r) => r.category !== "Stort tapp")
                ...
                <div ref={groups} className="scroll-mt-5 space-y-3">
                  <div className="flex items-center justify-between">
                    <h3 className="font-black">{filter ?? "Omgång för omgång"}</h3>
                    {filter && (
                      <button
                  ...
                  {[1, 2].map((group) => {
                    const selected = rows.filter(
                      (r) => r.group === group && (!filter || r.zone.category === filter),
                    );
                    return selected.length ? (
                      <section
                        key={group}
                        className="overflow-hidden rounded-2xl border border-slate-200"
                        aria-label={`Omgång ${group}`}
                      >
                        <h4 className="bg-blue-50 p-3 font-black">Omgång {group}</h4>
                        {selected.map((r) => (
                          <div
                            key={r.ball}
                            className="flex items-center justify-between gap-2 border-t border-slate-100 p-3 text-sm"
                          >
                            <div>
                              <p className="font-bold">
                                Boll {r.ball} · {r.zone.label}
                              </p>
                              <span
                                className={`mt-1 inline-flex rounded-full px-2 py-1 text-xs font-bold ${r.zone.tone}`}
                              >
                                {r.zone.category}
                              </span>
                            </div>
                            <strong className="shrink-0 text-blue-700">+{r.points}</strong>
                          </div>
                        ))}
                      </section>
                    ) : null;
                  ...
                  <p className="text-xs text-slate-500">
                    Grov uppskattning från resultatets avståndszoner. Den valda bunkern påverkar
                    svårigheten.
                  </p>
                </div>
              </div>
          ...
          <div className="mx-auto flex h-full max-w-md flex-col justify-center p-6 text-center">
            ...
            <p className="mt-2 text-slate-500">
              Se ditt estimerade handicap och analysen för varje omgång med SG4+.
            </p>
