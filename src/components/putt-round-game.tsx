...
export function PuttRoundGame({
  ...
  return (
    ...
    >
      <style>{`.putt-primary,.putt-secondary,.putt-black{display:flex;width:100%;min-height:52px;align-items:center;justify-content:center;border-radius:16px;padding:12px 16px;font-weight:800}.putt-primary{background:#2563eb;color:white}.putt-black{background:#111827;color:white}.putt-secondary{background:white;border:1px solid #e2e8f0;color:#1e40af}.putt-primary:disabled{opacity:.45}.putt-playing{display:flex;flex-direction:column;gap:10px}.putt-playing .putt-map>div{height:clamp(145px,24dvh,220px);margin:0}.putt-star{animation:puttStar .55s cubic-bezier(.2,.8,.3,1.2) both}.putt-average-win{animation:puttShine 1.2s ease-out 2}@keyframes puttStar{0%{opacity:0;transform:scale(.4)}65%{transform:scale(1.17)}100%{opacity:1;transform:scale(1)}}@keyframes puttShine{50%{box-shadow:0 0 28px #60a5fa66;transform:scale(1.02)}}@media(max-height:700px){.putt-playing{gap:8px}.putt-playing .putt-map>div{height:145px}.putt-playing .putt-hole{padding:10px}.putt-playing .putt-hole svg{height:36px;width:36px}.putt-playing .putt-entry{min-height:60px}}@media(prefers-reduced-motion:reduce){.putt-star,.putt-average-win{animation:none}}`}</style>
      {active ? (
        <>
          <header className="flex min-h-16 overflow-hidden rounded-2xl border border-blue-600 bg-white shadow-sm">
            ...
            <div className="flex flex-1 items-center justify-end gap-3 px-3 text-blue-700">
              ...
              <div className="text-center">
                <strong className="text-lg">{completed}/6</strong>
          ...
          {active.phase === "halfway" ? (
            <section className="space-y-4 rounded-3xl border bg-white p-5 text-center">
              ...
              <strong className="block text-4xl text-amber-500">{total} / 8 ★</strong>
              {pbGoal && (
              ...
              <section>
                ...
                <div className="grid grid-cols-4 gap-2">
                  {[1, 2, 3, 4].map((p) => (
                    <button
                      key={p}
                      className={`putt-entry min-h-20 rounded-2xl border-2 text-center disabled:opacity-60 ${shots[0] === p ? "border-blue-600 bg-blue-600 text-white" : "border-blue-200 bg-blue-50 text-blue-800"}`}
        ...
        <div className="space-y-4 pt-5">
          {(isPB || roundTotal === 16) && fresh === round.id && <ChipCelebration />}
          <div className="text-center">
            ...
            <p className="mt-3 text-6xl font-black text-amber-500">
              {roundTotal}
              <span className="text-xl text-slate-400">
                {" "}
                / {round.holes.reduce((s, _, i) => s + maxStars(i), 0)} ★
              </span>
            </p>
          </div>
          <div className="grid grid-cols-6 gap-1 rounded-2xl bg-white p-3">
            {round.holes.map((h, i) => (
              <div key={i} className="text-center">
                <p className="mb-1 text-xs font-bold text-slate-500">Hål {i + 1}</p>
                <PuttStars count={holeStars(h, i)} max={maxStars(i)} zero={holeStars(h, i) === 0} />
              </div>
            ))}
          </div>
          {round.status === "full" && (
            <>
              <PuttMilestones stars={roundTotal} />
              <div
                className={`rounded-2xl bg-blue-50 p-4 text-center ${beatAverage && fresh === round.id ? "putt-average-win" : ""}`}
              >
            ...
            {state.history
              ...
              .map((r) => (
                ...
                >
                  ...
                  <strong className="text-xl text-amber-600">{roundStars(r)} ★ →</strong>
                </button>
              ))}
          </div>
        </>
