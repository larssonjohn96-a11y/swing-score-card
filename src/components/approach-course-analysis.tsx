...
export function ApproachCourseAnalysis({ round }: { round: CourseRound }) {
  ...
  const review = buildActivityReview({
    title: "Rundanalys",
    handicap: hcp,
    modelId: "approach-course-v1",
    outcomes: rows.map((r) => ({
      label: `Hål ${r.hole}`,
      context: `Mål ${r.target} m`,
      result: `${r.miss.toFixed(1)} m från målet`,
      rank: r.miss / r.target,
    ...
    >
      ...
      <DialogContent className="!animate-none !fixed !inset-0 !left-0 !top-0 !h-[100dvh] !max-h-none !w-full !max-w-none !translate-x-0 !translate-y-0 !rounded-none !border-0 !p-0 !gap-0 overflow-hidden !bg-transparent text-slate-950 [&>button]:z-30 [&>button]:bg-white [&>button]:p-2">
        ...
        <DialogDescription className="sr-only">
          Ditt estimerade inspels-handicap och slag för slag, grupperat per hål.
        </DialogDescription>
          ...
          <>
            ...
            >
              <div className="max-h-[85dvh] w-full max-w-md space-y-4 overflow-y-auto rounded-[26px] border border-slate-200 bg-white p-5 shadow-xl">
                <div>
                  <h2 className="text-2xl font-black">Estimerat inspels-HCP</h2>
                ...
                <div className="grid grid-cols-2 gap-3 rounded-2xl bg-slate-50 p-3 text-center">
                  <div>
                    <strong className="text-2xl text-blue-700">
                      {averageMiss(round).toFixed(1).replace(".", ",")} m
                    </strong>
                    <p className="text-xs text-slate-500">Snittavvikelse från målet</p>
                  </div>
                  <div>
                        ...
                        <div className="space-y-2 p-3 text-sm">
                          ...
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <strong className="text-blue-700">
                              {r.miss.toFixed(1).replace(".", ",")} m från målet
                            </strong>
                          </div>
                        </div>
                      </section>
