...
import { approachAverage } from "@/lib/approach-course";
import { fetchFriendSnapshot, listFriendships, pushPlayerSnapshot } from "@/lib/friends-cloud";
import type { CourseRound } from "@/lib/approach-course";
type Row = ReturnType<typeof approachAverage> & { id: string; name: string };
export function ApproachLeaderboard({
  userId,
  history,
  playerName,
}: {
        ...
        const friends = await Promise.all(
          accepted.map(async (f) => {
            ...
            return {
              ...
              count: value("approach-round-count"),
              points: value("approach-round-stars"),
              stars: value("approach-round-stars"),
            };
          }),
        );
        if (alive) {
          ...
          return (
            ...
            >
              ...
              <div className="text-right">
                ...
                <p className="text-xs text-slate-500">{r.count ? `av 18 ★` : ""}</p>
              </div>
            </li>
          );
        })}
