...
import { COURSE_DISTANCES, holeStars, maxStars } from "@/lib/driver-course";
import { DriverStars as Stars } from "./driver-stars";
const positions = [
  [16, 27],
  [48, 25],
  [83, 27],
...
export function DriverCourseMap({
  holes,
  cursor,
  reference,
}: {
  holes: import("@/lib/driver-course").CourseShot[][];
  reference: number;
  ...
  return (
    ...
    >
      ...
      {COURSE_DISTANCES.map((d, i) => (
        <div
          key={i}
          className="driver-map-hole absolute flex -translate-x-1/2 -translate-y-1/2 flex-col items-center gap-0.5"
          style={{ left: `${positions[i][0]}%`, top: `${positions[i][1]}%` }}
        >
          <span
            aria-label={`Hål ${i + 1}, $±20 meter`}
      ...
      {point && (
        <div
          className="driver-map-golfer pointer-events-none absolute z-10 -translate-x-1/2 -translate-y-full transition-[left,top] duration-700 ease-in-out motion-reduce:transition-none"
          style={{
