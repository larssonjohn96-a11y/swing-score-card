...
import { type ApproachResult } from "@/lib/approach-match";
import { validShot } from "@/lib/driver-course";
export function DriverRoundEntry({
  target,
  initial,
  onScore,
}: {
  ...
  return (
    <section aria-label="Registrera drive">
      ...
      <p className="my-2 text-center text-sm font-bold text-blue-900" aria-live="polite">
        {valid
          ? `${shot.lateral <= 20 ? "Inom zonen" : "Utanför zonen"} · din längd ${target} m`
          : "Ange en giltig längd och sidled."}
      </p>
      <button className="driver-primary" disabled={!valid} onClick={() => onScore(shot)}>
        Registrera slag →
      </button>
    </section>
  );
