...
import { bunkerAverage } from "@/lib/bunker-course";
import { fetchFriendSnapshot, listFriendships, pushPlayerSnapshot } from "@/lib/friends-cloud";
import type { CourseRound } from "@/lib/bunker-course";
type Row = ReturnType<typeof bunkerAverage> & { id: string; name: string };
export function BunkerLeaderboard({
  userId,
  history,
  playerName,
}: {
        ...
        const friends = await Promise.all(
          accepted.map(async (f) => {
            ...
            return {
              ...
              count: value("bunker-round-count"),
              points: value("bunker-round-points"),
              stars: value("bunker-round-stars"),
            };
          }),
        );
        if (alive) {
        ...
        {all.map((r) => {
          const rank = r.count
            ? all.filter(
                (x) =>
                  x.count > 0 &&
                  (x.stars > r.stars || (x.stars === r.stars && x.points > r.points)),
              ).length + 1
            : null;
          return (
            <li
            ...
            >
              ...
              <div className="text-right">
                <p className="text-xl font-black text-blue-700">{r.count ? fmt(r.stars) : "–"}</p>
                <p className="text-xs text-slate-500">
                  {r.count ? `${fmt(r.points)} poäng · max 6 ★` : ""}
                </p>
              </div>
            </li>
          );
