...
function skillFromPct(pct: number) {
  return ratingFromHandicap(handicapFromPct(pct));
}
...
function proximityMetric(key: MetricKey, subject: string, shots: ApproachShotRecord[]): Metric {
  ...
  return {
    key,
    subject,
    score: shots.length ? skillFromPct(avgPct) : 0,
    raw: shots.length ? `${avgMeters.toFixed(1).replace(".", ",")} m` : "–",
...
function errorMetric(
  key: MetricKey,
  subject: string,
  shots: ApproachShotRecord[],
  getter: (shot: ApproachShotRecord) => number,
): Metric {
  const pct = avg(shots.map(getter));
  return {
    key,
    subject,
    score: shots.length ? skillFromPct(pct) : 0,
    raw: shots.length ? `${pct.toFixed(1).replace(".", ",")} % fel` : "–",
    attempts: shots.length,
  };
}

function RadarTooltip({ active, payload }: { active?: boolean; payload?: Array<{ payload?: Metric & { benchmarkLabel?: string } }> }) {
  ...
  return (
    <div className="rounded-xl border border-border bg-card px-3 py-2 text-xs shadow-lg">
      ...
      <p className="mt-1 text-muted-foreground">Du: <span className="font-semibold text-foreground">{row.raw}</span></p>
      <p className="text-muted-foreground">{row.attempts} registrerade slag</p>
    </div>
  );
}

export function ApproachRadarAnalysis() {
  ...
  const data = metrics.map((metric) => ({ ...metric, benchmark: benchmarkScore, benchmarkLabel: selectedBenchmark.label }));

  return (
    <section className="rounded-3xl border border-border bg-card p-4">
      <h2 className="font-display text-3xl">Analys approach</h2>
      ...
      <div className="flex items-center justify-center gap-5 text-[11px] text-muted-foreground">
        ...
        <span className="inline-flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-full bg-chart-3" />{selectedBenchmark.label}</span>
      </div>

      <div className="mt-4 grid grid-cols-2 gap-2">
        {metrics.map((metric) => (
          <div key={metric.key} className="rounded-2xl border border-border p-3">
            <p className="text-[10px] uppercase tracking-[0.14em] text-muted-foreground">{metric.subject}</p>
            <p className="mt-1 font-display text-2xl">{metric.raw}</p>
            <p className="text-[10px] text-muted-foreground">{metric.attempts} slag</p>
          </div>
        ))}
      </div>
    </section>
