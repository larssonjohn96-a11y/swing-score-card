...
import { approachAverage, roundStars, type CourseRound } from "@/lib/approach-course";
export function ApproachRoundImpact({
  round,
  before,
  userId,
  fresh,
  ...
  const previous = approachAverage(before),
    after = approachAverage([...before, round]);
      ...
      const friends = await Promise.all(
        accepted.map(async (f) => {
          ...
          return {
            ...
            stars: m.find((m) => m.key === "approach-round-stars")?.value ?? 0,
            count: m.find((m) => m.key === "approach-round-count")?.value ?? 0,
          };
        }),
      );
