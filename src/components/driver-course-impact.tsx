...
import { driverAverage, roundPoints, type CourseRound } from "@/lib/driver-course";
export function DriverRoundImpact({
  round,
  before,
  userId,
  fresh,
  ...
  const previous = driverAverage(before),
    after = driverAverage([...before, round]);
      ...
      const friends = await Promise.all(
        accepted.map(async (f) => {
          ...
          return {
            ...
            stars: m.find((m) => m.key === "driver-round-points")?.value ?? 0,
            count: m.find((m) => m.key === "driver-round-count")?.value ?? 0,
          };
        }),
      );
