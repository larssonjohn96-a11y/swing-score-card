...
import { roundStars, type CourseRound } from "@/lib/bunker-course";
export function BunkerProgress({ history }: { history: CourseRound[] }) {
  ...
  const graph = (interactive: boolean) => (
    <svg
      viewBox="0 0 310 185"
      role="img"
      aria-label="Stjärnor per hel runda, från 0 till 6"
      className="w-full"
    >
      {[0, 2, 4, 6].map((v) => (
        <g key={v}>
          <line
            ...
            stroke="#e2e8f0"
          />
          <text x="2" y={159 - v * (128 / 6)} fontSize="10" fill="#64748b">
            {v}
          </text>
        </g>
      ))}
        ...
        <g
          ...
          {...(interactive
            ? {
                role: "button",
                tabIndex: 0,
                "aria-label": `Runda ${i + 1}: ${values[i].toFixed(1).replace(".", ",")} stjärnor`,
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="!inset-0 !h-[100dvh] !w-full !max-w-none !translate-x-0 !translate-y-0 !rounded-none bg-white text-slate-950">
          <div className="m-auto w-full max-w-lg">
            ...
            <DialogDescription className="my-3">Stjärnor per hel runda · max 6</DialogDescription>
            {graph(true)}
            <p className="mt-4 text-center font-bold text-blue-700">
              {selected !== null && rounds[selected]
                ? `${new Date(rounds[selected].finishedAt).toLocaleDateString("sv-SE")} · ${values[selected].toFixed(1).replace(".", ",")} av 6 ★`
                : values.length
