...
export const DRIVER_ROUND_TEST_ID = "driver-course-round";
export function driverSession(userId: string, round: CourseRound): TestSession {
  return {
    id: deterministicUuid(`${DRIVER_ROUND_TEST_ID}:${userId}:${round.id}`),
    testId: DRIVER_ROUND_TEST_ID,
    category: "driving",
    testType: "training",
...
export function mergeDriverRounds(local: CourseState, remote: CourseRound[]): CourseState {
...
export async function reconcileDriverRounds(
  user: string,
  port: DriverCloudPort,
): Promise<CourseState> {
  const check = async () => {
  ...
  const merged = mergeDriverRounds(port.readLocal(user), remote);
  port.writeLocal(user, merged);
  return merged;
}
const queues = new Map<string, Promise<CourseState>>();
export function syncDriverRounds(user: string): Promise<CourseState> {
  ...
  const run = (
    ...
    return reconcileDriverRounds(user, {
      currentUser: async () => {
      ...
      writeLocal: (id, state) => {
        ...
        window.dispatchEvent(
          new CustomEvent("sg4-driver-cloud-updated", { detail: { userId: id } }),
        );
      },
      upload: (id, rounds) =>
        supabaseGateway.upsert(
          id,
          rounds.map((r) => driverSession(id, r)),
        ),
      fetch: async (id) => {
        ...
        for (let from = 0; ; from += 500) {
          const { data, error } = await supabase
            ...
            .select("metrics")
            .eq("user_id", id)
            .eq("test_id", DRIVER_ROUND_TEST_ID)
            .order("id")
            .range(from, from + 499);
