...
import {
  CHIP_STATIONS, bestAt, chipStorageKey, emptyChipProgress, goalAt, masteryAt, parseChipProgress,
  pointsForLeave, recentAt, recommendStation, reduceChipProgress, roundTotal, sessionRounds,
  unlockedDistances, type ChipPoints, type ChipProgress,
} from './chip-stations';
let sequence = 0;
const start = () => reduceChipProgress(emptyChipProgress(), { type: 'start', id: `session-${sequence++}`, lies: ['Fairway', 'Ruff'], at: 1 });
function play(state: ChipProgress, distance: number, shots: ChipPoints[]) {
  let next = reduceChipProgress(state, { type: 'begin', id: `round-${sequence++}`, distance, lie: 'Fairway', at: sequence });
...
describe('unlocks, mastery and recommendations', () => {
  ...
  it('does not let a direct selection bypass a lock or abandon a partial round', () => {
    let p = start();
    expect(reduceChipProgress(p, { type: 'begin', id: 'no', distance: 30, lie: 'Fairway', at: 0 })).toBe(p);
    p = play(p, 8, [2]);
    expect(reduceChipProgress(p, { type: 'stations' })).toBe(p);
    expect(reduceChipProgress(p, { type: 'begin', id: 'no', distance: 8, lie: 'Fairway', at: 0 })).toBe(p);
  });
  ...
  it('has strictly increasing mastery targets with no chip-in required for unlock', () => {
    for (const s of CHIP_STATIONS) {
      ...
      for (let i = 1; i < s.tiers.length; i++) expect(s.tiers[i].points).toBeGreaterThan(s.tiers[i - 1].points);
    }
  });
});
...
describe('undo, persistence and pass boundaries', () => {
  ...
  it('deduplicates persisted rounds and ignores broken or legacy 0–5 data', () => {
    ...
    const restored = parseChipProgress(JSON.stringify({ ...p, rounds: [...p.rounds, p.rounds[0], bad] }));
    expect(restored.rounds).toHaveLength(1);
