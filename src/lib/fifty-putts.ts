...
export function calculateFiftyPuttSession(entries:FiftyPuttEntry[],createdAt=new Date().toISOString()):FiftyPuttSession{const total=entries.reduce((sum,e)=>sum+e.strokes,0);const byDistance=FIFTY_PUTT_DISTANCES.map(distance=>{const strokes=entries.filter(e=>e.distance===distance).reduce((sum,e)=>sum+e.strokes,0);const par=FIFTY_PUTT_PAR_BY_DISTANCE[distance];return{distance,strokes,par,versusPar:strokes-par}});return{id:`${createdAt}-${Math.random().toString(36).slice(2,8)}`,createdAt,total,versusPar:total-FIFTY_PUTT_PAR,byDistance,entries}}
...
export function saveFiftyPuttSession(entries:FiftyPuttEntry[]):FiftyPuttSession{const session=calculateFiftyPuttSession(entries);if(typeof window!=="undefined"){const sessions=loadFiftyPuttSessions();window.localStorage.setItem(STORAGE_KEY,JSON.stringify([...sessions,session]))}return session}
