...
export const BUNKER_ROUND_TEST_ID = "bunker-course-round";
export function bunkerSession(userId: string, round: CourseRound): TestSession {
  return {
    id: deterministicUuid(`${BUNKER_ROUND_TEST_ID}:${userId}:${round.id}`),
    testId: BUNKER_ROUND_TEST_ID,
...
export function mergeBunkerRounds(local: CourseState, remote: CourseRound[]): CourseState {
...
export async function reconcileBunkerRounds(
  user: string,
  port: BunkerCloudPort,
): Promise<CourseState> {
  const check = async () => {
  ...
  const merged = mergeBunkerRounds(port.readLocal(user), remote);
  port.writeLocal(user, merged);
  return merged;
}
const queues = new Map<string, Promise<CourseState>>();
export function syncBunkerRounds(user: string): Promise<CourseState> {
  ...
  const run = (
    ...
    return reconcileBunkerRounds(user, {
      currentUser: async () => {
      ...
      writeLocal: (id, state) => {
        ...
        window.dispatchEvent(
          new CustomEvent("sg4-bunker-cloud-updated", { detail: { userId: id } }),
        );
      },
      upload: (id, rounds) =>
        supabaseGateway.upsert(
          id,
          rounds.map((r) => bunkerSession(id, r)),
        ),
      fetch: async (id) => {
        ...
        for (let from = 0; ; from += 500) {
          const { data, error } = await supabase
            ...
            .select("metrics")
            .eq("user_id", id)
            .eq("test_id", BUNKER_ROUND_TEST_ID)
            .order("id")
            .range(from, from + 499);
