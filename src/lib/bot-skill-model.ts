...
export function simulateChipBotResult(distance: number, categoryHcp: number, archetype?: BotArchetype, random = Math.random): ChipBotResult {
  ...
  const disasterChance = clamp(
    (0.015 + positiveHcp * 0.0065 + Math.max(0, distance - 12) * 0.0035) * styleDisaster,
    0.008,
    0.38,
  );
...
export function simulatePuttingBotStrokes(distance: number, categoryHcp: number, archetype?: BotArchetype, random = Math.random) {
...
export function simulateDriveBotResult(categoryHcp: number, archetype?: BotArchetype, random = Math.random): DriveBotResult {
  const positiveHcp = Math.max(0, categoryHcp);
  const aggression = styleValue(archetype, "aggression");
  const consistency = styleValue(archetype, "consistency");
