...
function Avatar({ name, url, large = false }: { name:string; url?:string|null; large?:boolean }) {
  ...
  return <div className={`flex ${size} shrink-0 items-center justify-center overflow-hidden rounded-full border-2 border-primary bg-primary/10 font-display text-xl text-primary`}>
  ...
  return <div className="overflow-hidden rounded-3xl border border-border bg-card">
    {rows.map((row, index) => {
      ...
      return <div key={row.key} className={`grid grid-cols-[1fr_1.35fr_1fr] items-center gap-2 px-4 py-4 ${index ? "border-t border-border/70" : ""}`}>
        <div className="text-left"><span className={`inline-flex min-w-14 justify-center rounded-xl px-2.5 py-1.5 text-base font-bold tabular-nums ${win === "left" ? "bg-primary text-primary-foreground" : "text-foreground"}`}>{hcp ? formatHcp(row.left) : formatValue(row.left,row.unit,row.decimals)}</span></div>
        <div className="text-center text-xs font-semibold uppercase tracking-[0.08em] text-muted-foreground">{row.label}</div>
        <div className="text-right"><span className={`inline-flex min-w-14 justify-center rounded-xl px-2.5 py-1.5 text-base font-bold tabular-nums ${win === "right" ? "bg-primary text-primary-foreground" : "text-foreground"}`}>{hcp ? formatHcp(row.right) : formatValue(row.right,row.unit,row.decimals)}</span></div>
      </div>;
    })}
  </div>;
}
...
function CompareFriendPage() {
  ...
  return <main className="mx-auto min-h-screen w-full max-w-md px-5 pb-28 pt-7">
    <header className="flex items-center justify-between">
      <Link to="/vanner" className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-border bg-card" aria-label="Tillbaka"><ArrowLeft className="h-4 w-4" /></Link>
    ...
    <section className="mt-7 grid grid-cols-[1fr_auto_1fr] items-center gap-3">
      <div className="flex min-w-0 flex-col items-center text-center"><Avatar name={selfName} url={selfAvatar} large/><p className="mt-2 max-w-[8rem] truncate text-sm font-bold">{selfName}</p><p className="mt-0.5 text-xs text-muted-foreground">HCP {formatHcp(local?.total)}</p></div>
      <div className="flex flex-col items-center"><span className="rounded-xl bg-foreground px-3 py-2 font-display text-2xl text-background">VS</span><span className="mt-2 font-display text-3xl tabular-nums">{score.left}–{score.right}</span></div>
      <div className="flex min-w-0 flex-col items-center text-center"><Avatar name={friend?.displayName ?? "Vän"} url={friend?.avatarUrl} large/><p className="mt-2 max-w-[8rem] truncate text-sm font-bold">{friend?.displayName ?? "Vän"}</p><p className="mt-0.5 text-xs text-muted-foreground">HCP {formatHcp(friendSnapshot?.estHcp ?? undefined)}</p></div>
    </section>
