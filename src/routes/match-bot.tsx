...
const BOTS: BotProfile[] = [
  { id: "margaret", name: "Margaret", hcp: 42, gender: "Kvinna", role: "Grandma golfer", tier: "Nybörjare", avatar: "👵🏻", intro: "Jag spelar lugnt, men underskatta inte mina korta puttar.", putting: 4, shortGame: 2, approach: -2, driving: -6 },
  { id: "leo", name: "Leo", hcp: 34, gender: "Man", role: "Ny golfare", tier: "Nybörjare", avatar: "🧑🏻", intro: "Jag började nyligen. Några riktigt bra slag dyker upp ibland.", putting: -1, shortGame: -1, approach: 0, driving: 3 },
  { id: "sarah", name: "Sarah", hcp: 27, gender: "Kvinna", role: "Weekend golfer", tier: "Nybörjare", avatar: "👩🏼", intro: "Helggolfare. Stabil när jag hittar rytmen.", putting: 2, shortGame: 0, approach: 0, driving: 1 },
  { id: "zach", name: "Zach", hcp: 22, gender: "Man", role: "Weekend golfer", tier: "Klubbspelare", avatar: "🧔🏻", intro: "Jag gillar att slå långt. Precisionen får vi se hur det går med.", putting: -2, shortGame: -1, approach: 0, driving: 5 },
  { id: "anna", name: "Anna", hcp: 16, gender: "Kvinna", role: "Klubbspelare", tier: "Klubbspelare", avatar: "👩🏻", intro: "Jag ger sällan bort ett hål. Du får vinna det.", putting: 2, shortGame: 2, approach: 1, driving: 0 },
  { id: "marcus", name: "Marcus", hcp: 11, gender: "Man", role: "Tävlingsgolfare", tier: "Klubbspelare", avatar: "👨🏽", intro: "Jag spelar aggressivt och går för flaggan.", putting: -1, shortGame: 1, approach: 3, driving: 3 },
  { id: "emma", name: "Emma", hcp: 7, gender: "Kvinna", role: "Singelhandicap", tier: "Avancerad", avatar: "👩🏼‍🦱", intro: "Fairways, greener och tålamod. Jag gör inte många stora misstag.", putting: 2, shortGame: 2, approach: 2, driving: 1 },
  { id: "ryan", name: "Ryan", hcp: 3, gender: "Man", role: "College player", tier: "Avancerad", avatar: "🧑🏽", intro: "Collegegolf. Jag kommer att pressa dig från första slaget.", putting: 2, shortGame: 2, approach: 4, driving: 5 },
  { id: "maya", name: "Maya", hcp: 0, gender: "Kvinna", role: "Elitamatör", tier: "Avancerad", avatar: "👩🏾", intro: "Scratch. Jag räknar med att du träffar ditt bästa slag.", putting: 3, shortGame: 4, approach: 4, driving: 3 },
  { id: "noah", name: "Noah", hcp: -2, gender: "Man", role: "College standout", tier: "Elit", avatar: "🧑🏼‍🦰", intro: "Jag spelar för att vinna. Pars räcker inte alltid.", putting: 4, shortGame: 3, approach: 5, driving: 5 },
  { id: "sofia", name: "Sofia", hcp: -4, gender: "Kvinna", role: "Tour prospect", tier: "Elit", avatar: "👩🏻‍🦰", intro: "Små marginaler. Ett svagt slag och jag tar hålet.", putting: 5, shortGame: 5, approach: 5, driving: 4 },
  { id: "alex", name: "Alex", hcp: -6, gender: "Man", role: "Tour level", tier: "Elit", avatar: "👨🏻", intro: "Tour-nivå. Du behöver spela nära ditt tak för att slå mig.", putting: 5, shortGame: 5, approach: 6, driving: 6 },
];
...
function BotMatchPage() {
  ...
  const bot = BOTS.find((item) => item.id === botId) ?? BOTS[3];
  ...
  function buildHoles() {
    ...
    setHoles(next); setHoleIndex(0); setYourValue(null); setDriveHit(true); setTurnState("you"); setStep("play");
  }
  ...
  async function register() {
    ...
    setTurnState("reveal");
    await sleep(1400);
    setYourValue(null); setDriveHit(true); setTurnState("you");
  ...
  return <main style={LIGHT_SURFACE} className="mx-auto min-h-screen w-full max-w-md bg-background px-5 pb-16 pt-6 text-foreground">
    ...
    {step === "bot" ? <>
      ...
      <div className="mt-5 space-y-6">{BOT_TIERS.map((tier) => <section key={tier}><p className="mb-2 px-1 text-[10px] font-black uppercase tracking-[0.16em] text-slate-500">{tier}</p><div className="grid grid-cols-2 gap-3">{BOTS.filter((item) => item.tier === tier).map((item) => {
        const active = item.id === botId;
        return <button key={item.id} onClick={() => setBotId(item.id)} className={`relative min-h-[176px] overflow-hidden rounded-[26px] border p-4 text-left transition ${active ? selected : glass}`}>
          {active ? <span className="absolute right-3 top-3 flex h-6 w-6 items-center justify-center rounded-full bg-emerald-600 text-white"><Check className="h-4 w-4" /></span> : null}
          <span className="text-5xl leading-none">{item.avatar}</span><span className="mt-3 block font-display text-2xl">{item.name}</span><span className="mt-1 block text-xs font-bold text-slate-700">HCP {formatHcp(item.hcp)}</span><span className="mt-1 block text-[11px] text-slate-500">{item.gender} · {item.role}</span>
        </button>;
      })}</div></section>)}</div>
