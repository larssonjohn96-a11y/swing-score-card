...
import { ArrowLeft, ArrowRight, BarChart3, Check, RotateCcw, Trophy, X, XIcon } from "lucide-react";
...
export const Route = createFileRoute("/tutor-test")({
  head: () => ({ meta: [{ title: "Tutor – Putting Start Line | SG4" }] }),
  component: TutorTestPage,
});
...
function TutorTestPage() {
    ...
    return (
      ...
      >
        ...
        <p className="mt-5 text-[10px] font-semibold uppercase tracking-[0.22em] text-muted-foreground">
          Putting · Startlinje · Träningstest
        </p>
        <h1 className="mt-2 font-display text-3xl leading-none">Tutor</h1>
        <p className="mt-2.5 text-[13px] leading-relaxed text-muted-foreground">
          Slå 10 puttar genom Putting Tutor. Varje boll som startar genom gaten ger 1 poäng.
          Testet isolerar din startlinje och repeterbarhet.
        </p>
        ...
        <section className="mt-5 rounded-2xl border border-border bg-card p-4">
          ...
          <div className="mt-3 space-y-2 text-sm">
            <div className="flex items-center justify-between border-b border-border pb-2">
              <span className="text-muted-foreground">Antal puttar</span>
              <span className="font-semibold">10</span>
            </div>
            <div className="flex items-center justify-between border-b border-border pb-2">
              <span className="text-muted-foreground">Genom Tutor</span>
              <span className="font-semibold text-primary">1 poäng</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-muted-foreground">Missar gaten</span>
              <span className="font-semibold">0 poäng</span>
            </div>
          </div>
        </section>

        <section className="mt-3 rounded-2xl border border-primary/20 bg-tint p-4">
          <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-primary">
            Viktigaste KPI:n
          </p>
          <p className="mt-2 font-display text-2xl leading-none">20-testers snitt</p>
          <p className="mt-2 text-xs leading-relaxed text-muted-foreground">
            Appen följer ditt rullande snitt över de senaste 20 testerna. Ett värde som 9,90 betyder
            i snitt 9,90 lyckade av 10 över de senaste 20 testerna.
          </p>
        </section>

        <button
          onClick={start}
          className="mt-auto flex w-full items-center justify-center gap-2 rounded-2xl bg-primary py-4 font-display text-xl text-primary-foreground"
        >
    ...
    return (
      ...
      >
        <div className="flex items-center justify-between pt-[max(1rem,env(safe-area-inset-top))]">
          <span className="text-sm font-semibold">Putt {shot + 1} av {TUTOR_PUTTS}</span>
          <Link
            to="/traning"
        ...
        <div className="mt-4 rounded-2xl border border-border bg-card p-3 shadow-sm">
          <div className="grid grid-cols-10 gap-1.5">
            {Array.from({ length: TUTOR_PUTTS }, (_, index) => {
        ...
        <section className="mt-4 flex h-[190px] flex-col items-center justify-center rounded-3xl border border-border bg-card px-6 text-center shadow-sm">
          <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-muted-foreground">
            Startlinje
          </p>
          <p className="mt-3 font-display text-4xl leading-none">Genom Tutor?</p>
          <p className="mt-3 max-w-[260px] text-sm leading-relaxed text-muted-foreground">
            Registrera bara om bollen startade genom gaten. Om putten gick i hål spelar ingen roll här.
          </p>
        </section>
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          <button
            ...
            className="flex h-[132px] flex-col items-center justify-center rounded-3xl border border-primary/20 bg-tint text-primary shadow-sm transition-transform active:scale-95"
          >
            <Check className="h-7 w-7" />
            <span className="mt-3 font-display text-2xl leading-none">Genom</span>
            <span className="mt-1 text-xs">1 poäng</span>
          </button>
          <button
            onClick={() => register(false)}
            className="flex h-[132px] flex-col items-center justify-center rounded-3xl border border-border bg-card shadow-sm transition-transform active:scale-95"
          >
            <XIcon className="h-7 w-7 text-muted-foreground" />
            <span className="mt-3 font-display text-2xl leading-none">Miss</span>
            <span className="mt-1 text-xs text-muted-foreground">0 poäng</span>
          </button>
        </div>
        ...
        <div className="mt-3 flex items-center justify-between rounded-2xl border border-border bg-card px-4 py-3 text-sm shadow-sm">
          ...
          <span className="font-semibold tabular-nums">{score} / {shot}</span>
        </div>
      </main>
    );
  }
  ...
  const isPerfect = score === TUTOR_PUTTS;
    ...
    >
      ...
      <section className="mt-5 rounded-3xl border border-border bg-card p-6 text-center">
        ...
        <p className="mt-2 font-display text-6xl leading-none text-primary">
          {score}<span className="ml-2 text-xl text-muted-foreground">/ 10</span>
        </p>
        {isPerfect ? (
      ...
      <section className="mt-3 rounded-3xl border border-primary/20 bg-tint p-5">
        <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-primary">
          {sessions.length >= TUTOR_ROLLING_WINDOW ? "20-testers snitt" : `Snitt hittills · ${rollingCount} test`}
        </p>
        <p className="mt-2 font-display text-5xl leading-none text-primary">
          {formatTutorAverage(latestAverage)}
        </p>
        <p className="mt-2 text-xs text-muted-foreground">
          {sessions.length >= TUTOR_ROLLING_WINDOW
            ? "Rullande snitt av dina senaste 20 test."
            : `20-testers KPI låses upp efter ${TUTOR_ROLLING_WINDOW - sessions.length} test till.`}
        </p>
      </section>

      <div className="mt-3 grid grid-cols-2 gap-3">
        <div className="rounded-2xl border border-border bg-card p-4">
          <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">Träffprocent</p>
          <p className="mt-2 font-display text-3xl leading-none">{score * 10}%</p>
          <p className="mt-1 text-xs text-muted-foreground">Det här testet</p>
        </div>
        <div className="rounded-2xl border border-border bg-card p-4">
          <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">PB · 20-test snitt</p>
          <p className="mt-2 font-display text-3xl leading-none">
            {bestAverage === null ? "–" : formatTutorAverage(bestAverage)}
          </p>
          <p className="mt-1 text-xs text-muted-foreground">
            {bestAverage === null ? "Kräver 20 test" : "Bästa rullande snitt"}
          </p>
        </div>
      </div>
