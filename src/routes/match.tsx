...
function MatchPlayPage() {
  ...
  function toggleFriend(id: string) {
    setSelectedFriendIds((ids) => ids.includes(id) ? ids.filter((x) => x !== id) : ids.length + guests.length >= neededOthers ? ids : [...ids, id]);
  }
  function addGuest() {
  ...
  function reset() {
    setMode(null); setSelectedFriendIds([]); setGuests([]); setGuestName(""); setBlueMateId(null); setCategory(null); setMatchType(null); setMatchLength(5); setHoles([]); setHoleIndex(0); setFinalText(""); setStep("players");
  }
  function back() {
    if (step === "teams") setStep("players");
    else if (step === "category") setStep(mode === "singles" ? "players" : "teams");
    else if (step === "type") setStep("category");
    else if (step === "length") setStep("type");
  }
  ...
  return <main style={LIGHT_SURFACE} className="mx-auto min-h-screen w-full max-w-md bg-background px-5 pb-16 pt-6 text-foreground">
    {step !== "play" && step !== "result" ? <header className="flex items-center justify-between">
      ...
      <div className="text-center"><p className="text-[9px] font-bold uppercase tracking-[0.22em] text-slate-500">SG4 Match Play</p><p className="text-[11px] font-semibold text-slate-700">{step === "players" ? "1 · Format & spelare" : step === "teams" ? "2 · Lag" : step === "category" ? "Kategori" : step === "type" ? "Matchtyp" : "Matchlängd"}</p></div>
      <Link to="/" className={`inline-flex h-10 w-10 items-center justify-center rounded-full border ${glass}`}>×</Link>
    </header> : null}

    {step === "players" ? <>
      <section className="mt-5"><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500">Match Play</p><h1 className="mt-1 font-display text-4xl leading-none">Välj format</h1></section>
      <div className="mt-5 grid grid-cols-2 gap-3">
        <button onClick={() => chooseMode("singles")} className={`col-span-2 rounded-[30px] border p-5 text-left ${mode === "singles" ? "border-slate-900 bg-slate-900 text-white shadow-lg" : glass}`}><span className="block font-display text-3xl">Singles</span><span className={`mt-1 block text-[10px] font-bold uppercase tracking-[0.14em] ${mode === "singles" ? "text-white/65" : "text-slate-500"}`}>1 mot 1 · klassisk match play</span></button>
        <button onClick={() => chooseMode("fourball")} className={`rounded-3xl border p-4 text-left ${mode === "fourball" ? "border-slate-900 bg-slate-900 text-white" : glass}`}><span className="block font-display text-xl">Fourball</span><span className={`mt-1 block text-[9px] font-bold uppercase ${mode === "fourball" ? "text-white/65" : "text-slate-500"}`}>2 mot 2 · bästa boll</span></button>
        <button onClick={() => chooseMode("foursomes")} className={`rounded-3xl border p-4 text-left ${mode === "foursomes" ? "border-slate-900 bg-slate-900 text-white" : glass}`}><span className="block font-display text-xl">Foursomes</span><span className={`mt-1 block text-[9px] font-bold uppercase ${mode === "foursomes" ? "text-white/65" : "text-slate-500"}`}>2 mot 2 · vartannat slag</span></button>
      </div>

      {mode ? <>
        <div className="mt-6 flex items-center justify-between"><h2 className="font-display text-2xl">Spelare</h2><span className="text-[10px] font-bold uppercase tracking-[0.14em] text-slate-500">{mode === "singles" ? "1 mot 1" : "4 spelare"}</span></div>
        {mode === "singles" ? <section className="mt-3 grid grid-cols-[1fr_auto_1fr] items-center gap-2">
          <div className={`flex min-h-44 flex-col items-center justify-center rounded-[28px] border p-4 text-center ${blueGlass}`}><PlayerAvatar player={selfPlayer} tone="blue" large /><p className="mt-3 max-w-full truncate text-sm font-bold">{selfName}</p><p className="mt-1 text-[10px] font-bold uppercase tracking-[0.14em] text-blue-600">Du · Blue</p></div>
          <span className="rounded-xl bg-slate-950 px-2.5 py-2 font-display text-xl text-white">VS</span>
          {selectedOthers[0] ? <div className={`relative flex min-h-44 flex-col items-center justify-center rounded-[28px] border p-4 text-center ${redGlass}`}><PlayerAvatar player={selectedOthers[0]} tone="red" large /><p className="mt-3 max-w-full truncate text-sm font-bold">{selectedOthers[0].name}</p><p className="mt-1 text-[10px] font-bold uppercase tracking-[0.14em] text-red-600">Red</p>{selectedOthers[0].isGuest ? <button onClick={() => removeGuest(selectedOthers[0].id)} className="absolute right-3 top-3 rounded-full bg-white/75 p-1.5 text-slate-500"><X className="h-3.5 w-3.5" /></button> : null}</div> : <button onClick={() => setPickerOpen(true)} className={`flex min-h-44 flex-col items-center justify-center rounded-[28px] border p-4 text-center ${redGlass}`}><span className="flex h-16 w-16 items-center justify-center rounded-full border-[3px] border-dashed border-red-400 text-red-500"><Plus className="h-6 w-6" /></span><p className="mt-3 font-display text-xl">Välj spelare</p><p className="mt-1 text-[10px] text-slate-500">Vän eller gäst</p></button>}
        </section> : <>
          <button onClick={() => setPickerOpen(true)} className={`mt-3 flex w-full items-center gap-4 rounded-3xl border p-4 text-left ${selectedOthers.length ? redGlass : glass}`}><Users className="h-5 w-5 text-red-600" /><span className="min-w-0 flex-1"><span className="block font-display text-xl">Välj tre spelare</span><span className="text-[10px] text-slate-500">{selectedOthers.length}/3 valda</span></span><ChevronRight className="h-5 w-5 text-slate-500" /></button>
          {selectedOthers.length ? <div className="mt-3 flex flex-wrap gap-2">{selectedOthers.map((p) => <span key={p.id} className="rounded-full border border-slate-300/70 bg-white/70 px-3 py-2 text-xs font-semibold">{p.name}</span>)}</div> : null}
        </>}
    ...
    {step === "category" ? <><section className="mt-5"><p className="text-[10px] font-bold uppercase text-slate-500">Välj kategori</p><h1 className="mt-1 font-display text-4xl">Vad ska ni spela?</h1></section><div className="mt-5 space-y-3">{CATEGORIES.map((i) => { const active = category === i.id; return <button key={i.id} onClick={() => { setCategory(i.id); setMatchType(null); }} className={`flex w-full items-center gap-4 rounded-3xl border p-5 text-left ${active ? "border-slate-900 bg-slate-900 text-white shadow-lg" : glass}`}><span className="min-w-0 flex-1"><span className="block font-display text-2xl">{i.title}</span><span className={`text-[11px] font-semibold ${active ? "text-white/75" : "text-slate-700"}`}>{i.subtitle}</span><span className={`mt-1 block text-xs ${active ? "text-white/60" : "text-slate-600"}`}>{i.description}</span></span>{active ? <Check className="h-5 w-5" /> : null}</button>; })}</div><button disabled={!category} onClick={() => setStep("type")} className="mt-6 flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-blue-600 via-slate-900 to-red-600 py-4 font-display text-xl text-white disabled:opacity-30">Nästa <ChevronRight className="h-5 w-5" /></button></> : null}
    ...
    {step === "play" && current ? <><header className="flex items-center justify-between"><button onClick={() => setStep("length")} className={`inline-flex h-10 w-10 items-center justify-center rounded-full border ${glass}`}><ArrowLeft className="h-4 w-4" /></button><div className="text-center"><p className="text-[9px] font-bold uppercase text-slate-500">{mode}</p><p className="font-display text-xl">{selectedType?.title}</p></div><span className="rounded-full border border-slate-300/80 bg-white/68 px-3 py-2 text-[10px] font-bold">Hål {holeIndex + 1}/{matchLength}</span></header><section className="mt-5 grid grid-cols-[1fr_auto_1fr] items-center gap-2"><div className={`rounded-3xl border p-3 text-center ${blueGlass}`}><p className="text-[9px] font-bold uppercase text-blue-600">Blue</p><p className="mt-1 truncate text-xs font-bold">{blueLabel}</p></div><div className="text-center"><p className="text-[9px] font-bold uppercase text-slate-500">Match</p><p className="mt-1 rounded-xl bg-slate-950 px-3 py-2 font-display text-2xl text-white">{liveStatus(diff)}</p></div><div className={`rounded-3xl border p-3 text-center ${redGlass}`}><p className="text-[9px] font-bold uppercase text-red-600">Red</p><p className="mt-1 truncate text-xs font-bold">{redLabel}</p></div></section><section className="mt-5 rounded-[32px] border border-blue-300/55 bg-gradient-to-br from-blue-100/78 via-white/78 to-red-50/58 p-6 text-center shadow-[0_24px_52px_-34px_rgba(37,99,235,.48)] backdrop-blur-2xl"><Target className="mx-auto h-6 w-6 text-blue-600" /><p className="mt-3 text-[10px] font-bold uppercase text-blue-600">{current.challenge.eyebrow}</p><h1 className="mt-1 font-display text-5xl">{current.challenge.title}</h1><p className="mt-3 text-xs text-slate-600">{current.challenge.detail}</p></section><div className="mt-5 flex items-center justify-between"><h2 className="font-display text-2xl">Vem vann hålet?</h2><span className="text-[10px] font-bold uppercase text-slate-500">{score.played} spelade</span></div><div className="mt-3 grid grid-cols-2 gap-3"><button onClick={() => recordWinner("blue")} className={`rounded-3xl border p-5 font-display text-xl text-blue-700 ${blueGlass}`}>{blueLabel}</button><button onClick={() => recordWinner("red")} className={`rounded-3xl border p-5 font-display text-xl text-red-700 ${redGlass}`}>{redLabel}</button></div><button onClick={() => recordWinner("tie")} className={`mt-3 w-full rounded-2xl border py-3.5 text-sm font-bold ${glass}`}>Delat hål · AS</button></> : null}
