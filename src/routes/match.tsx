...
function MatchPlayPage() {
  ...
  function startMatch() {
    ...
    setHoleIndex(0); setBlueStrokes(1); setRedStrokes(1); setFinalText(""); setIsSubmitting(false); setTransitionMessage(null); setEditingHoleIndex(null); setReturnHoleIndex(null); setStep("play");
  }
  ...
  function recordPutting() {
    ...
    if (editingHoleIndex !== null) {
      ...
      window.setTimeout(() => {
        ...
        setBlueStrokes(targetHole?.blueStrokes ?? 1);
        setRedStrokes(targetHole?.redStrokes ?? 1);
  ...
  function editPuttingHole(index: number) {
    if (isSubmitting || !isPutting) return;
    const hole = holes[index];
    if (typeof hole?.blueStrokes !== "number" || typeof hole?.redStrokes !== "number") return;
    setReturnHoleIndex(holeIndex); setEditingHoleIndex(index); setHoleIndex(index); setBlueStrokes(hole.blueStrokes); setRedStrokes(hole.redStrokes);
    window.scrollTo({ top: 0, behavior: "smooth" });
  }
  function reset() {
    setMode(null); setSelectedFriendIds([]); setGuests([]); setGuestName(""); setBlueMateId(null); setCategory(null); setMatchType(null);
    setScoringMode("match"); setMatchLength(5); setHoles([]); setHoleIndex(0); setFinalText(""); setBlueStrokes(1); setRedStrokes(1); setIsSubmitting(false); setTransitionMessage(null); setEditingHoleIndex(null); setReturnHoleIndex(null); setStep("players");
  }
  function back() {
