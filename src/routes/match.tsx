...
function MatchPlayPage() {
  ...
  function toggleFriend(id: string) { setSelectedFriendIds((ids) => ids.includes(id) ? ids.filter((x) => x !== id) : ids.length + guests.length >= neededOthers ? ids : [...ids, id]); }
  function addGuest() { const name = guestName.trim(); if (!name || selectedOthers.length >= neededOthers) return; setGuests((old) => [...old, { id: `guest-${Date.now()}-${old.length}`, name, isGuest: true }]); setGuestName(""); }
  ...
  return <main style={LIGHT_SURFACE} className="mx-auto min-h-screen w-full max-w-md bg-background px-5 pb-16 pt-6 text-foreground">
    ...
    {step === "category" ? <><section className="mt-5"><p className="text-[10px] font-bold uppercase text-slate-500">Välj kategori</p><h1 className="mt-1 font-display text-4xl">Vad ska ni spela?</h1></section><div className="mt-5 grid grid-cols-2 gap-3">{CATEGORIES.map((i) => { const active = category === i.id; return <button key={i.id} onClick={() => { setCategory(i.id); setMatchType(null); }} className={`relative min-h-36 rounded-[26px] border p-4 text-left ${active ? "border-slate-900 bg-slate-900 text-white shadow-lg" : glass}`}><span className={`block text-[9px] font-bold uppercase tracking-[0.16em] ${active ? "text-white/60" : "text-slate-500"}`}>{i.subtitle}</span><span className="mt-2 block font-display text-2xl leading-none">{i.title}</span><span className={`mt-2 block text-[11px] leading-relaxed ${active ? "text-white/60" : "text-slate-600"}`}>{i.description}</span>{active ? <span className="absolute right-3 top-3 flex h-6 w-6 items-center justify-center rounded-full bg-white text-slate-950"><Check className="h-4 w-4" /></span> : null}</button>; })}</div><button disabled={!category} onClick={() => setStep("type")} className="mt-6 flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-blue-600 via-slate-900 to-red-600 py-4 font-display text-xl text-white disabled:opacity-30">Nästa <ChevronRight className="h-5 w-5" /></button></> : null}
