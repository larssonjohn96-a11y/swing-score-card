...
function MatchPlayPage() {
  ...
  function reset() {
    ...
    setScoringMode("match"); setMatchLength(5); setHoles([]); setHoleIndex(0); setFinalText(""); setBlueStrokes(1); setRedStrokes(1); setBluePoints(null); setRedPoints(null); setShortGameLies([]); setIsSubmitting(false); setTransitionMessage(null); setEditingHoleIndex(null); setReturnHoleIndex(null); setStep("players");
  }
  function back() {
  ...
  return <main style={LIGHT_SURFACE} className="mx-auto min-h-screen w-full max-w-md bg-background px-5 pb-16 pt-6 text-foreground">
    ...
    {step === "setup" && isShortGame ? <><section className="mt-5"><p className="text-[10px] font-bold uppercase text-slate-500">Närspel</p><h1 className="mt-1 font-display text-4xl">Setup</h1><p className="mt-2 text-sm text-slate-600">Välj vilka lies som ska ingå.</p></section><div className="mt-5 rounded-3xl border border-slate-300/80 bg-slate-100/80 p-5 text-center shadow-[0_16px_36px_-30px_rgba(15,23,42,.35)]"><p className="text-[9px] font-bold uppercase tracking-[0.16em] text-slate-500">Spelform</p><p className="mt-1 font-display text-2xl text-slate-900">Closest to the Pin</p><p className="mt-1 text-xs text-slate-600">Ett slag per spelare · högst poäng vinner hålet.</p></div><div className="mt-6 flex items-center justify-between"><h2 className="font-display text-2xl">Välj lies</h2><span className="text-[10px] font-bold uppercase tracking-[0.14em] text-slate-500">Välj minst en</span></div><div className="mt-3 grid grid-cols-3 gap-2">{SHORT_GAME_LIES.map((item) => { const active = shortGameLies.includes(item.id); return <button key={item.id} onClick={() => toggleShortGameLie(item.id)} className={`flex min-h-20 items-center justify-center gap-2 rounded-2xl border px-3 py-4 text-center ${active ? selectedGlass : glass}`}><span className="text-sm font-bold">{item.title}</span>{active ? <Check className="h-4 w-4 shrink-0 text-blue-600" /> : null}</button>; })}</div>{!setupValid ? <p className="mt-2 text-center text-[10px] font-bold text-amber-700">Välj minst ett lie.</p> : null}<button disabled={!setupValid} onClick={() => setStep("length")} className="mt-5 flex w-full items-center justify-center gap-2 rounded-2xl bg-slate-950 py-4 font-display text-xl text-white disabled:opacity-30">Nästa <ChevronRight className="h-5 w-5" /></button></> : null}
