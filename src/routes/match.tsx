...
type Step = "players" | "teams" | "category" | "type" | "scoring" | "length" | "play" | "result";
...
type Hole = { challenge: Challenge; winner: HoleWinner; blueStrokes?: number; redStrokes?: number };
...
const CATEGORIES = [
  ...
  { id: "putting", title: "Putting", subtitle: "Puttning", description: "Håla ut och låt SG4 räkna resultatet automatiskt." },
] as const;
...
const MATCH_TYPES: Record<MatchCategory, Array<{ id: string; title: string; description: string }>> = {
  ...
  putting: [
    { id: "short", title: "Short Putts", description: "1–5 m. Håla ut från varje avstånd. Lägst antal slag vinner." },
    { id: "mix", title: "Mixed Putting", description: "1–10 m. SG4 blandar avstånden. Håla ut varje hål." },
    { id: "lag", title: "Lag Putting", description: "8–20 m. Håla ut och undvik treputtar." },
  ],
};
...
function generateChallenge(category: MatchCategory, typeId: string, mode: MatchMode): Challenge {
  const suffix = mode === "fourball" ? " · registrera lagets bästa score" : mode === "foursomes" ? " · laget spelar vartannat slag" : "";
  if (category === "putting") {
    const distance = typeId === "short" ? rand(1, 5) : typeId === "lag" ? rand(8, 20) : rand(1, 10);
    const eyebrow = typeId === "short" ? "Short Putts" : typeId === "lag" ? "Lag Putting" : "Mixed Putting";
    return { eyebrow, title: `${distance} m`, detail: `Håla ut · lägst antal slag vinner${suffix}` };
  }
...
function matchScore(holes: Hole[]) {
  return holes.reduce((s, h) => {
    ...
    return s;
  }, { blue: 0, red: 0, played: 0, blueStrokes: 0, redStrokes: 0 });
}
...
function MatchPlayPage() {
  ...
  function startMatch() {
    ...
    setHoleIndex(0); setBlueStrokes(1); setRedStrokes(1); setFinalText(""); setStep("play");
  }
  ...
  function back() {
    ...
    else if (step === "length") setStep(isPutting ? "scoring" : "type");
  }

  const stepLabel = step === "players" ? "1 · Format & spelare" : step === "teams" ? "2 · Lag" : step === "category" ? "Kategori" : step === "type" ? "Spel" : step === "scoring" ? "Spelform" : "Matchlängd";
  ...
  return <main style={LIGHT_SURFACE} className="mx-auto min-h-screen w-full max-w-md bg-background px-5 pb-16 pt-6 text-foreground">
    ...
    {step === "play" && current ? <><header className="flex items-center justify-between"><button onClick={() => setStep("length")} className={`inline-flex h-10 rounded-full border px-4 items-center justify-center text-xs font-bold ${glass}`}>Inställningar</button><div className="text-center"><p className="text-[9px] font-bold uppercase text-slate-500">{isPutting ? (scoringMode === "match" ? "Match Play" : "Stroke Play") : mode}</p><p className="font-display text-xl">{selectedType?.title}</p></div><span className="rounded-full border border-slate-300/80 bg-white/68 px-3 py-2 text-[10px] font-bold">Hål {holeIndex + 1}/{matchLength}</span></header>
      ...
      {isPutting ? <><div className="mt-5 flex items-end justify-between"><div><p className="text-[9px] font-bold uppercase tracking-[0.16em] text-slate-500">Registrera score</p><h2 className="mt-1 font-display text-2xl">Hur många slag?</h2></div><span className="text-[10px] font-semibold text-slate-500">Håla ut först</span></div><div className="mt-3 grid grid-cols-2 gap-3">{([[blueLabel, blueStrokes, setBlueStrokes, "blue"], [redLabel, redStrokes, setRedStrokes, "red"]] as const).map(([label, value, setter, tone]) => <div key={tone} className={`rounded-[28px] border p-4 text-center ${tone === "blue" ? blueGlass : redGlass}`}><p className={`truncate text-xs font-bold ${tone === "blue" ? "text-blue-700" : "text-red-700"}`}>{label}</p><div className="mt-4 flex items-center justify-between gap-2"><button onClick={() => setter(Math.max(1, value - 1))} className="flex h-11 w-11 items-center justify-center rounded-full border border-white/80 bg-white/75 shadow-sm"><Minus className="h-4 w-4" /></button><div><p className={`font-display text-5xl leading-none ${tone === "blue" ? "text-blue-700" : "text-red-700"}`}>{value}</p><p className="mt-1 text-[9px] font-bold uppercase text-slate-500">slag</p></div><button onClick={() => setter(Math.min(9, value + 1))} className="flex h-11 w-11 items-center justify-center rounded-full border border-white/80 bg-white/75 shadow-sm"><Plus className="h-4 w-4" /></button></div></div>)}</div><button onClick={recordPutting} className="mt-4 flex w-full items-center justify-center gap-2 rounded-2xl bg-slate-950 py-4 font-display text-xl text-white">Registrera hålet <ChevronRight className="h-5 w-5" /></button></> : <><div className="mt-5 flex items-center justify-between"><h2 className="font-display text-2xl">Vem vann hålet?</h2><span className="text-[10px] font-bold uppercase text-slate-500">{score.played} spelade</span></div><div className="mt-3 grid grid-cols-2 gap-3"><button onClick={() => recordWinner("blue")} className={`rounded-3xl border p-5 font-display text-xl text-blue-700 ${blueGlass}`}>{blueLabel}</button><button onClick={() => recordWinner("red")} className={`rounded-3xl border p-5 font-display text-xl text-red-700 ${redGlass}`}>{redLabel}</button></div><button onClick={() => recordWinner("tie")} className={`mt-3 w-full rounded-2xl border py-3.5 text-sm font-bold ${glass}`}>Delat hål · AS</button></>}

      <section className={`mt-5 rounded-[26px] border p-4 ${glass}`}><div className="flex items-center justify-between"><div><p className="text-[9px] font-bold uppercase tracking-[0.16em] text-slate-500">{scoringMode === "stroke" && isPutting ? "Scorekort" : "Matchkort"}</p><h3 className="mt-1 font-display text-xl">Hål för hål</h3></div><p className="text-right text-[10px] font-semibold text-slate-500">{score.played} spelade · {holesRemaining} kvar</p></div><div className="mt-3 flex gap-1.5 overflow-x-auto pb-1">{holes.map((hole, index) => { const active = index === holeIndex; const cls = hole.winner === "blue" ? "border-blue-500 bg-blue-500 text-white" : hole.winner === "red" ? "border-red-500 bg-red-500 text-white" : hole.winner === "tie" ? "border-slate-400 bg-slate-200 text-slate-700" : active ? "border-slate-900 bg-slate-900 text-white" : "border-slate-300 bg-white/70 text-slate-500"; return <div key={index} className={`flex h-10 min-w-10 flex-col items-center justify-center rounded-xl border text-[9px] font-bold leading-none ${cls}`}><span>{index + 1}</span>{isPutting && hole.blueStrokes && hole.redStrokes ? <span className="mt-1 text-[8px] opacity-85">{hole.blueStrokes}-{hole.redStrokes}</span> : null}</div>; })}</div>{isPutting && scoringMode === "stroke" ? <div className="mt-3 flex justify-between text-[10px] font-bold"><span className="text-blue-600">{blueLabel}: {score.blueStrokes}</span><span className="text-red-600">{redLabel}: {score.redStrokes}</span></div> : <div className="mt-3 flex items-center gap-4 text-[9px] font-bold uppercase tracking-[0.12em] text-slate-500"><span className="flex items-center gap-1.5"><i className="h-2 w-2 rounded-full bg-blue-500" />Blue</span><span className="flex items-center gap-1.5"><i className="h-2 w-2 rounded-full bg-red-500" />Red</span><span className="flex items-center gap-1.5"><i className="h-2 w-2 rounded-full bg-slate-300" />Delat</span></div>}</section>
      {isPutting && scoringMode === "stroke" && score.played > 0 ? <p className="mt-3 text-center text-[10px] font-semibold text-slate-500">{strokeDiff === 0 ? "Delat totalt" : `${strokeDiff > 0 ? blueLabel : redLabel} leder med ${Math.abs(strokeDiff)} slag`}</p> : null}</> : null}

    {step === "result" ? <><section className="mt-10 rounded-[34px] border border-slate-300/80 bg-gradient-to-br from-blue-100/72 via-white/80 to-red-100/64 p-6 text-center shadow-[0_24px_56px_-32px_rgba(15,23,42,.5)] backdrop-blur-2xl"><Trophy className="mx-auto h-8 w-8" /><p className="mt-4 text-[10px] font-bold uppercase text-slate-500">{isPutting && scoringMode === "stroke" ? "Stroke Play Result" : "Match Result"}</p><h1 className="mt-2 font-display text-4xl">{finalText}</h1>{isPutting ? <div className="mt-5 grid grid-cols-2 gap-3"><div className={`rounded-2xl border p-3 ${blueGlass}`}><p className="truncate text-xs font-bold text-blue-700">{blueLabel}</p><p className="mt-1 font-display text-3xl text-blue-700">{score.blueStrokes}</p><p className="text-[9px] uppercase text-blue-600">slag</p></div><div className={`rounded-2xl border p-3 ${redGlass}`}><p className="truncate text-xs font-bold text-red-700">{redLabel}</p><p className="mt-1 font-display text-3xl text-red-700">{score.redStrokes}</p><p className="text-[9px] uppercase text-red-600">slag</p></div></div> : null}</section><button onClick={reset} className="mt-5 flex w-full items-center justify-center gap-2 rounded-2xl bg-slate-950 py-4 font-display text-xl text-white"><RotateCcw className="h-5 w-5" /> Ny match</button></> : null}
