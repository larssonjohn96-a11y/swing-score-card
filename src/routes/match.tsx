...
function MatchPlayPage() {
  ...
  return <main style={LIGHT_SURFACE} className={`mx-auto min-h-screen w-full max-w-md bg-background px-5 text-foreground ${step === "play" ? "pb-4 pt-2" : "pb-16 pt-6"}`}>
    {(step === "play" || step === "sudden-death") ? <button type="button" onClick={() => setAbortConfirmOpen(true)} className="fixed right-4 top-[max(10px,env(safe-area-inset-top))] z-[45] rounded-full border border-red-200/90 bg-white/92 px-3 py-2 text-[10px] font-black uppercase tracking-[0.08em] text-red-700 shadow-sm backdrop-blur-xl">Avbryt spel</button> : null}
