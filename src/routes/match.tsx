...
function MatchPlayPage() {
  ...
  function recordWinner(w: Exclude<HoleWinner, null>) { advance(holes.map((h, i) => i === holeIndex ? { ...h, winner: w } : h)); }
  ...
  const stepLabel = step === "players" ? "1 · Spelform & spelare" : step === "teams" ? "2 · Lag" : step === "category" ? "Kategori" : step === "type" ? "Spel" : "Matchlängd";
  ...
  return <main style={LIGHT_SURFACE} className="mx-auto min-h-screen w-full max-w-md bg-background px-5 pb-16 pt-6 text-foreground">
    ...
    {step === "play" && current ? <><header className="flex items-center justify-between"><span className="w-16" /><div className="text-center"><p className="text-[9px] font-bold uppercase text-slate-500">{scoringMode === "match" ? "Match Play" : "Stroke Play"}</p><p className="font-display text-xl">{selectedCategory?.title}</p></div><span className="rounded-full border border-slate-300/80 bg-white/68 px-3 py-2 text-[10px] font-bold">Hål {holeIndex + 1}/{matchLength}</span></header>
      ...
      <section className={`mt-5 overflow-hidden rounded-[24px] border ${glass}`}><div className="flex items-center justify-between border-b border-slate-200/80 px-4 py-3"><div><p className="text-[9px] font-bold uppercase tracking-[0.16em] text-slate-500">{scoringMode === "stroke" ? "Scorekort" : "Matchkort"}</p><h3 className="mt-0.5 font-display text-xl">Hål för hål</h3></div><p className="text-right text-[10px] font-semibold text-slate-500">{score.played} spelade · {holesRemaining} kvar</p></div><div className="overflow-x-auto"><div className="min-w-max"><div className="grid" style={{ gridTemplateColumns: `minmax(94px,1.35fr) repeat(${matchLength},44px)` }}><div className="border-b border-r border-slate-200 bg-slate-100 px-3 py-2 text-[10px] font-bold text-slate-600">Hål</div>{holes.map((_, i) => <div key={`h-${i}`} className="border-b border-r border-slate-200 bg-slate-100 py-2 text-center text-[10px] font-bold text-slate-700">{i + 1}</div>)}<div className="border-b border-r border-slate-200 px-3 py-2 text-[10px] font-bold text-blue-700 truncate">{blueLabel}</div>{holes.map((h, i) => <div key={`b-${i}`} className="border-b border-r border-slate-200 py-2 text-center text-xs font-bold text-blue-700">{h.blueStrokes ?? "–"}</div>)}<div className="border-b border-r border-slate-200 px-3 py-2 text-[10px] font-bold text-red-700 truncate">{redLabel}</div>{holes.map((h, i) => <div key={`r-${i}`} className="border-b border-r border-slate-200 py-2 text-center text-xs font-bold text-red-700">{h.redStrokes ?? "–"}</div>)}<div className="border-r border-slate-200 bg-slate-50 px-3 py-2 text-[10px] font-bold text-slate-600">Ställning</div>{holes.map((h, i) => <div key={`s-${i}`} className="border-r border-slate-200 bg-slate-50 py-2 text-center text-[9px] font-bold text-slate-700">{h.winner === null ? (i === holeIndex ? "•" : "–") : scoringMode === "match" ? (() => { let d = 0; for (let x = 0; x <= i; x++) { if (holes[x]?.winner === "blue") d++; if (holes[x]?.winner === "red") d--; } return liveStatus(d); })() : (() => { let b = 0; let r = 0; for (let x = 0; x <= i; x++) { b += holes[x]?.blueStrokes ?? 0; r += holes[x]?.redStrokes ?? 0; } const delta = r - b; return delta === 0 ? "AS" : delta > 0 ? `${delta} före` : `${Math.abs(delta)} efter`; })()}</div>)}</div></div></div></section>
