...
type MatchCategory = "off-the-tee" | "approach" | "around-the-green" | "bunker" | "putting";
  ...
  return <main style={LIGHT_SURFACE} className={`mx-auto min-h-screen w-full max-w-md bg-background px-5 text-foreground ${step === "play" ? "pb-4 pt-2" : "pb-16 pt-6"}`}>
    ...
    {step === "players" ? <>
      {entryFlow === "friend" ? <>
        ...
        <button disabled={!canContinuePlayers} onClick={() => goToStep("category")} className={`sg4-ryder-next mt-6 ${ryderNext}`}><span aria-hidden="true" className="sg4-ryder-sweep pointer-events-none absolute inset-y-[-30%] left-[-35%] w-[38%] bg-white/70 blur-[1px]" />Nästa <ChevronRight className="h-5 w-5" /></button>
      </> : <>
    ...
    {step === "length" && selectedType ? <>{isPutting ? <>
      ...
      <button onClick={startMatch} className="relative mt-6 flex w-full items-center justify-center gap-2 overflow-hidden rounded-2xl border border-white/80 bg-[linear-gradient(90deg,rgba(254,226,226,.94)_0%,rgba(255,255,255,.88)_50%,rgba(219,234,254,.94)_100%)] py-4 font-display text-xl text-slate-900 shadow-[inset_0_1px_0_rgba(255,255,255,.98),0_14px_34px_-22px_rgba(148,163,184,.65)] backdrop-blur-xl transition active:scale-[.985]"><Flag className="h-5 w-5" /> Starta</button>
    </> : <><section className="mt-5"><p className="text-[10px] font-bold uppercase text-slate-500">{isPgaPutting ? "PGA Tour Putting" : scoringMode === "match" ? "Match Play" : "Slagspel"}</p><h1 className="mt-1 font-display text-4xl">{isShortGameScoring ? "Välj antal hål" : isPgaPutting ? "Matchlängd" : scoringMode === "stroke" ? "Antal hål" : "Bäst av"}</h1>{isPgaPutting ? <p className="mt-2 text-sm text-slate-600">Välj 3, 5 eller 7 hål. Alla längder får en mix av korta, mellanlånga och långa PGA Tour-avstånd.</p> : null}</section><div className="mt-5 grid grid-cols-3 gap-3">{([3, 5, 7] as const).map((v) => <button key={v} onClick={() => setMatchLength(v)} className={`relative rounded-3xl border px-3 py-6 ${matchLength === v ? selectedRing : glass}`}>{matchLength === v ? <SelectedCheck className="absolute right-1.5 top-1.5" /> : null}<span className="block whitespace-nowrap font-display text-3xl leading-none">{v} <span className="text-xl">hål</span></span>{v === 3 ? <span className="mt-2 block text-[9px] font-bold uppercase tracking-[0.12em] text-slate-500">Snabb</span> : v === 5 ? <span className="mt-2 block text-[9px] font-bold uppercase tracking-[0.12em] text-blue-600">Standard</span> : null}</button>)}</div><button onClick={startMatch} className="relative mt-6 flex w-full items-center justify-center gap-2 overflow-hidden rounded-2xl border border-white/80 bg-[linear-gradient(90deg,rgba(254,226,226,.94)_0%,rgba(255,255,255,.88)_50%,rgba(219,234,254,.94)_100%)] py-4 font-display text-xl text-slate-900 shadow-[inset_0_1px_0_rgba(255,255,255,.98),0_14px_34px_-22px_rgba(148,163,184,.65)] backdrop-blur-xl transition active:scale-[.985]"><Flag className="h-5 w-5" /> Starta</button></>}</> : null}
