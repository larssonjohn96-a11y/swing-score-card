  ...
  head: () => ({
    meta: [
      { title: "Träningstester | SG4" },
      {
        name: "description",
        content:
          "Färdigbyggda träningstester för off the tee, approach, around the green och putting. Följ din utveckling utan att påverka SG4 HCP.",
      },
    ],
  }),
  ...
  "off-the-tee": [
    {
      ...
      meta: "16 drives · 30 m fairway · konsekvens",
    },
  ],
  approach: [
    {
  ...
  return (
    <main className="mx-auto min-h-screen w-full max-w-md px-5 pb-28 pt-10">
      <header>
        ...
        <h1 className="mt-2 flex items-center gap-2 font-display text-4xl leading-none"><Dumbbell className="h-7 w-7 text-primary" />Träna & följ progress</h1>
        <p className="mt-3 text-sm text-muted-foreground">Färdigbyggda tester för sådant golfare redan tränar på. De påverkar inte ditt SG4 HCP.</p>
      </header>
        ...
        <section className="mt-7">
          ...
          <div className="mt-3 space-y-3">
            {CATEGORIES.map((item) => {
              ...
              return (
                <Link key={item.id} to="/traning" search={{ category: item.id }} className="flex w-full items-center gap-4 rounded-3xl border border-border bg-card p-4 text-left shadow-[var(--shadow-glow)] transition-colors hover:border-primary">
                  <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-tint-strong text-primary"><Target className="h-5 w-5" /></span>
                  <span className="min-w-0 flex-1"><span className="block font-display text-2xl leading-none">{item.title}</span><span className="mt-1 block text-xs text-muted-foreground">{item.description}</span><span className="mt-2 block text-[10px] font-semibold uppercase tracking-[0.16em] text-primary">{count} {count === 1 ? "färdigt test" : "färdiga tester"}</span></span>
                  <ChevronRight className="h-5 w-5 shrink-0 text-muted-foreground" />
                </Link>
              );
            })}
