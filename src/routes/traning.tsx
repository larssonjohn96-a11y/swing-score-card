...
function TestCard({
  ...
  to: "/lagputt-test" | "/50-bollar" | "/kategori/$slug";
  title: string;
  description: string;
  meta: string;
}) {
  ...
  return (
    <main className="mx-auto min-h-screen w-full max-w-md px-5 pb-28 pt-10">
            ...
            {CATEGORIES.map((item) => (
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className={`mt-2 block text-[10px] font-semibold uppercase tracking-[0.16em] ${item.count ? "text-primary" : "text-muted-foreground"}`}>
                    {item.count ? `${item.count} ${item.count === 1 ? "färdigt test" : "färdiga tester"}` : "Fler test kommer"}
                  </span>
                </span>
                <ChevronRight className="h-5 w-5 shrink-0 text-muted-foreground" />
              </button>
          ...
          {category === "putting" ? (
            <div className="mt-4 space-y-3">
              <TestCard
                to="/lagputt-test"
                title="Lag Putt"
                description="Träna längdkontroll på långa puttar från 8 till 18 meter. Resultatet sparas så att du kan följa utvecklingen."
                meta="6 puttar · 8–18 meter"
              />
              <TestCard
                to="/50-bollar"
                title="50-bollsövningen"
                description="50 puttar från 1 till 5 meter. Räkna varje slag tills alla bollar är hålade och försök slå ditt personbästa."
      ...
      <div className="mt-10 border-t border-border pt-5">
        <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">Behöver du något eget?</p>
        <p className="mt-1 text-xs text-muted-foreground">
