...
function GroupSessionPage() {
  ...
  if (loadError && !session) return (
    <main style={LIGHT_SURFACE} className="mx-auto min-h-screen w-full max-w-md bg-background px-5 pb-10 pt-10 text-foreground">
      ...
      <button type="button" onClick={() => { setLoading(true); setLoadError(null); void refresh(); }} className="mt-5 flex w-full items-center justify-center gap-2 rounded-2xl bg-primary py-4 font-semibold text-primary-foreground"><RotateCcw className="h-4 w-4" /> Försök igen</button>
