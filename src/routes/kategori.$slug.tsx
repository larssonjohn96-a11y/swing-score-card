...
import { findCategory, type Category, type CategoryTest } from "@/lib/categories";
...
export const Route = createFileRoute("/kategori/$slug")({
  ...
  head: ({ loaderData }) =>
    loaderData
      ? {
          meta: [
            { title: `${loaderData.category.title} – golftester` },
            {
              name: "description",
              content: `${loaderData.category.description} Kör testerna och följ utvecklingen över tid.`,
            },
          ],
        }
      : { meta: [{ title: "Kategorin hittades inte" }] },
  notFoundComponent: () => (
    <main className="mx-auto min-h-screen w-full max-w-md px-5 pt-16">
      <h1 className="text-4xl">Kategorin finns inte</h1>
      <Link to="/tester" className="mt-6 inline-block rounded-full border border-border px-4 py-2 text-sm">
        Till Train &amp; Test
      </Link>
    </main>
  ),
...
function CategoryPage() {
  ...
  useEffect(() => {
    ...
    const lo = offtee.at(-1);

    setLast({
      "/fairway": fw.length
        ? `Senast ${fw.at(-1)!.points.toFixed(0)} p · ${(fairwayHitRate(fw.at(-1)!.drives) * 100).toFixed(0)}% fairway`
        : undefined,
  ...
  const coreTests =
    category.slug === "puttning"
      ? category.tests.filter((t) => t.to !== "/lagputt-test" && t.to !== "/50-bollar")
      : category.tests;

  return (
    <main className="relative mx-auto min-h-screen w-full max-w-md px-5 pb-16 pt-[max(92px,calc(env(safe-area-inset-top)+72px))]">
      <Link
        to="/tester"
        aria-label="Tillbaka"
        className="absolute left-5 top-[max(16px,env(safe-area-inset-top))] z-20 flex h-14 w-14 items-center justify-center rounded-full border border-white/35 bg-white/18 text-foreground shadow-[0_8px_28px_rgba(15,23,42,.14)] backdrop-blur-2xl supports-[backdrop-filter]:bg-white/14"
      >
      ...
      <header>
        ...
        <h1 className="mt-1 text-4xl leading-none">{category.title}</h1>
      </header>
      ...
      <section className="mt-8">
        <div className="mb-4">
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-flag">Testa din nivå</p>
          <h2 className="mt-1 text-2xl">HCP-test</h2>
          <p className="mt-1 text-xs text-muted-foreground">Gör testet och se vilken HCP-nivå du motsvarar i kategorin.</p>
        </div>
        ...
        <div className="space-y-4">
          {coreTests.map((t: CategoryTest) => {
            ...
            return (
              <Link key={t.to} to={t.to} className="block rounded-3xl border border-border bg-card p-5">
                <div className="flex h-32 items-center justify-center overflow-hidden rounded-2xl bg-primary/5">
                  {thumb ? thumb() : <Target className="h-8 w-8 text-flag" />}
                </div>
                <div className="mt-4 flex items-center justify-between gap-3">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.2em] text-flag">HCP-resultat</p>
                    <h3 className="mt-1 text-3xl">{CARD_TITLES[t.to] ?? t.title}</h3>
                  </div>
                  <ChevronRight className="h-5 w-5 shrink-0 text-muted-foreground" />
                </div>
                <p className="mt-2 text-sm text-muted-foreground">{t.subtitle}</p>
                {last[t.to] && <p className="mt-2 text-xs text-muted-foreground">{last[t.to]}</p>}
              </Link>
            );
          })}
        </div>
