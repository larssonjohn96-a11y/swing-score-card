...
export const Route = createFileRoute("/kategori/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    return { meta: [{ title: `${title} – golftester` }, { name: "description", content: `${description} Kör testerna och följ utvecklingen över tid.` }] };
  },
...
function CategoryNotFound() {
  return <main className="mx-auto min-h-screen w-full max-w-md px-5 pt-16"><h1 className="text-4xl">Kategorin finns inte</h1><Link to="/" className="mt-6 inline-block rounded-full border border-border px-4 py-2 text-sm">Till menyn</Link></main>;
}
...
function CategoryPage() {
  ...
  useEffect(() => {
    ...
    setLast({
      "/fairway": fw.length ? `Senast ${fw[fw.length - 1].points.toFixed(0)} p · ${(fairwayHitRate(fw[fw.length - 1].drives) * 100).toFixed(0)}% fairway` : undefined,
      "/longdrive": ld.length ? `Senast ${sessionBest(ld[ld.length - 1]).toFixed(0)} ${ld[ld.length - 1].unit} längsta carry` : undefined,
      "/speed-test": sp.length ? `Senast ${sp[sp.length - 1].avgBallSpeed.toFixed(1)} mph ball speed` : undefined,
      "/bunker-test": d.length ? `Senast ${d[d.length - 1].avgProximity.toFixed(2)} m i snitt` : undefined,
      "/offtee-test": lastOfftee ? `Senast ${lastOfftee.score.toFixed(0)} / 100` : undefined,
    });
  }, []);

  const coreTests = category.slug === "puttning"
  ...
  return (
    <main className="mx-auto min-h-screen w-full max-w-md px-5 pb-16 pt-10">
      <header className="flex items-end justify-between">
        <div><p className="text-xs uppercase tracking-[0.25em] text-muted-foreground">{category.subtitle}</p><h1 className="text-4xl leading-none">{category.title}</h1></div>
        <Link to="/" className="rounded-full border border-border px-4 py-2 text-sm font-medium text-muted-foreground">Tillbaka</Link>
      </header>
      ...
      <section className="mt-8">
        <div className="mb-4">
          ...
          <p className="mt-1 text-xs text-muted-foreground">Resultaten används för din SG4-profil och HCP.</p>
        </div>
        <div className="space-y-4">
          {coreTests.map((t: CategoryTest) => {
            const thumb = TEST_THUMBNAILS[t.to];
            const title = CARD_TITLES[t.to] ?? t.title;
            return <Link key={t.to} to={t.to} className="block rounded-3xl border border-border bg-card p-5 shadow-[var(--shadow-glow)] transition-colors hover:border-primary">
              <div className="flex h-32 w-full items-center justify-center overflow-hidden rounded-2xl bg-primary/5">{thumb ? thumb() : <Target className="h-8 w-8 text-flag" />}</div>
      ...
      {category.slug === "puttning" ? (
        <section className="mt-10 border-t border-border pt-8">
          <div className="mb-4 flex items-start gap-3">
            <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-muted"><Dumbbell className="h-5 w-5 text-muted-foreground" /></span>
            <div><p className="text-xs font-semibold uppercase tracking-[0.22em] text-muted-foreground">Träningstester</p><h2 className="mt-1 text-2xl leading-none">Träna & följ progress</h2><p className="mt-1 text-xs text-muted-foreground">Benchmarks och övningar för träning. Påverkar inte SG4 HCP.</p></div>
          </div>
          <div className="space-y-3">
            {PUTTING_TRAINING_TESTS.map((test) => (
              <Link key={test.to} to={test.to} className="flex items-center gap-4 rounded-3xl border border-border bg-card p-4 shadow-[var(--shadow-glow)] transition-colors hover:border-primary">
                <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-muted"><Dumbbell className="h-5 w-5 text-muted-foreground" /></span>
                <span className="min-w-0 flex-1"><span className="block text-xl leading-none">{test.title}</span><span className="mt-1 block text-xs leading-snug text-muted-foreground">{test.subtitle}</span><span className="mt-2 block text-[10px] font-semibold uppercase tracking-[0.16em] text-muted-foreground">{test.meta}</span></span>
              </Link>
            ))}
          </div>
        </section>
