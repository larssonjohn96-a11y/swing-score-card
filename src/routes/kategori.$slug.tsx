...
const PUTTING_PROGRESS_TESTS = [
  {
    ...
    subtitle: "50 puttar från 1–5 meter.",
    meta: "Score · PR · utveckling",
  },
  {
    to: "/lagputt-test" as const,
    title: "Lag Putt",
    subtitle: "Längdkontroll från 8–18 meter.",
    meta: "Precision · PR · utveckling",
  },
];

const APPROACH_PROGRESS_TESTS = [
  {
    to: "/approach-pei-valj" as const,
    title: "PEI Precision",
    subtitle: "Wedge, järn eller total precision.",
    meta: "Score · PR · utveckling",
  },
];
...
const AROUND_GREEN_PROGRESS_TESTS = [
  {
    to: "/8-bollar" as const,
    title: "8-bollsövningen",
    subtitle: "Chip, pitch, lobb och bunker från åtta stationer.",
    meta: "Score · PR · utveckling",
  },
];
...
function CategoryPage() {
  ...
  useEffect(() => {
    ...
    setLast({
      "/fairway": fw.length
        ...
        : undefined,
      "/longdrive": ld.length
        ? `Senast ${sessionBest(ld.at(-1)!).toFixed(0)} ${ld.at(-1)!.unit} längsta carry`
        : undefined,
      "/speed-test": sp.length
        ? `Senast ${sp.at(-1)!.avgBallSpeed.toFixed(1)} mph ball speed`
        : undefined,
      "/bunker-test": d.length
        ? `Senast ${d.at(-1)!.avgProximity.toFixed(2)} m i snitt`
        : undefined,
  ...
  const progressTests =
    category.slug === "puttning"
      ? PUTTING_PROGRESS_TESTS
      : category.slug === "approach"
        ? APPROACH_PROGRESS_TESTS
        : category.slug === "around-the-green"
          ? AROUND_GREEN_PROGRESS_TESTS
          : [];
      ...
      <section className="mt-8">
        <div className="mb-4">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Gör testet och se vilken HCP-nivå du motsvarar i kategorin.
          </p>
        </div>
          ...
          {coreTests.map((t: CategoryTest) => {
            ...
            return (
              <Link
                key={t.to}
                to={t.to}
                className="block rounded-3xl border border-border bg-card p-5"
              >
                ...
                <div className="mt-4 flex items-center justify-between gap-3">
                  <div>
                    <p className="text-xs uppercase tracking-[0.3em] text-flag">HCP-resultat</p>
      ...
      {progressTests.length > 0 && (
        <section className="mt-10 border-t border-border pt-7">
          <div className="mb-3">
            ...
            <h2 className="mt-1 text-xl">Följ din utveckling</h2>
            <p className="mt-1 text-xs text-muted-foreground">
              Precision- och scoringtester för träning. De påverkar inte ditt HCP-resultat.
            </p>
          </div>

          <div className="overflow-hidden rounded-3xl border border-border bg-card">
            {progressTests.map((test, index) => (
              <Link
                key={test.to}
                to={test.to}
              ...
              >
                <span className="flex-1">
                  <span className="block text-lg">{test.title}</span>
                  <span className="mt-0.5 block text-xs text-muted-foreground">
                    {test.subtitle}
                  </span>
                  <span className="mt-1.5 block text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">
                    {test.meta}
                  </span>
                </span>
                <ChevronRight className="h-5 w-5 shrink-0 text-muted-foreground" />
