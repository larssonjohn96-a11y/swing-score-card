...
const CARD_TITLES: Partial<Record<CategoryTest["to"], string>> = {
  "/approach": "Inspelstest", "/offtee-test": "Off the Tee", "/speed-test": "Speed", "/bunker-test": "Bunkerslag", "/narspel-test": "Närspelstest", "/short-putting-test": "Short Putting", "/lagputt-test": "Lag Putt", "/putting": "Putting Test",
};
...
function CategoryPage(){
  ...
  useEffect(()=>{const d=loadBunkerSessions(),sp=loadSpeedSessions(),ld=loadLongDriveSessions(),fw=loadFairwaySessions(),offtee=loadOffTeeSessions(),lo=offtee.at(-1);setLast({"/fairway":fw.length?`Senast ${fw.at(-1)!.points.toFixed(0)} p · ${(fairwayHitRate(fw.at(-1)!.drives)*100).toFixed(0)}% fairway`:undefined,"/longdrive":ld.length?`Senast ${sessionBest(ld.at(-1)!).toFixed(0)} ${ld.at(-1)!.unit} längsta carry`:undefined,"/speed-test":sp.length?`Senast ${sp.at(-1)!.avgBallSpeed.toFixed(1)} mph ball speed`:undefined,"/bunker-test":d.length?`Senast ${d.at(-1)!.avgProximity.toFixed(2)} m i snitt`:undefined,"/offtee-test":lo?`Senast ${lo.score.toFixed(0)} / 100`:undefined})},[]);
  ...
  return <main className="mx-auto min-h-screen w-full max-w-md px-5 pb-16 pt-10">
    ...
    <section className="mt-8"><div className="mb-4"><p className="text-xs font-semibold uppercase tracking-[0.22em] text-flag">Testa din nivå</p><h2 className="mt-1 text-2xl">HCP-test</h2><p className="mt-1 text-xs text-muted-foreground">Gör testet och se vilken HCP-nivå du motsvarar i kategorin.</p></div><div className="space-y-4">{coreTests.map((t:CategoryTest)=>{const thumb=TEST_THUMBNAILS[t.to];return <Link key={t.to} to={t.to} className="block rounded-3xl border border-border bg-card p-5"><div className="flex h-32 items-center justify-center overflow-hidden rounded-2xl bg-primary/5">{thumb?thumb():<Target className="h-8 w-8 text-flag"/>}</div><div className="mt-4 flex items-center justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-flag">HCP-resultat</p><h3 className="mt-1 text-3xl">{CARD_TITLES[t.to]??t.title}</h3></div><ChevronRight className="h-5 w-5 shrink-0 text-muted-foreground"/></div><p className="mt-2 text-sm text-muted-foreground">{t.subtitle}</p>{last[t.to]&&<p className="mt-2 text-xs text-muted-foreground">{last[t.to]}</p>}</Link>})}</div></section>
