...
function CategoryDetailPage(){
 ...
 return <main className="mx-auto min-h-screen w-full max-w-md px-5 pb-28 pt-7">
  ...
  {slug==="around-the-green"&&<div className="mt-5"><AroundGreenRadarAnalysis/></div>}
  {detail.keyMetrics.length>0&&slug!=="approach"&&slug!=="around-the-green"&&<section className="mt-6"><p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-muted-foreground">Vad består din {detail.title.toLowerCase()} av?</p><div className="mt-3 grid grid-cols-2 gap-2">{detail.keyMetrics.map(m=><div key={m.label} className="rounded-2xl border border-border bg-card p-4"><p className="text-[10px] uppercase tracking-[0.14em] text-muted-foreground">{m.label}</p><p className="mt-1 font-display text-2xl">{m.value}</p></div>)}</div></section>}

  {detail.heatmap.length>0&&slug!=="approach"&&slug!=="around-the-green"&&<section className="mt-6"><p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-muted-foreground">Detaljer</p><div className="mt-3"><HeatmapCard title={slug==="puttning"?"Putting per avstånd":"Prestation per avstånd"} zones={detail.heatmap} unit={slug==="puttning"?"%":""}/></div></section>}
