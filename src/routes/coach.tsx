...
type CoachView = "overview" | "players" | "player-profile" | "focus-editor" | "progress" | "more";
...
function CoachPage() {
  ...
  async function loadCoachData() {
    ...
    const realStudents = coachRels.map((relationship) => {
      ...
      return {
        relationship,
        name: playerProfile?.display_name ?? "Spelare",
        gender: playerProfile?.gender ?? null,
        clubName: playerProfile?.club_name ?? null,
    ...
    const johnPreview: PlayerRow[] = isJohnMaster ? [{
      ...
      name: johnProfile?.display_name ?? displayName ?? "John",
      gender: johnProfile?.gender ?? null,
      clubName: johnProfile?.club_name ?? null,
  ...
  function openFocus(playerId: string) {
    setSelectedPlayerId(playerId);
    setView("focus-editor");
    window.scrollTo({ top: 0, behavior: "smooth" });
  }
  ...
  async function extendBlock(block: FocusBlock) {
    ...
    const start = new Date(`${block.end_date}T00:00:00`); start.setDate(start.getDate() + 1);
    const end = new Date(start); end.setDate(end.getDate() + durationDays - 1);
  ...
  const mainNavView = view === "player-profile" || view === "focus-editor" ? "players" : view;
      ...
      <div className="relative mx-auto w-full max-w-md px-5 pb-32 pt-6">
        <header className="flex items-center justify-between">
          ...
          <div className="relative">
            <button type="button" onClick={() => setRoleMenuOpen((open) => !open)} className={`flex items-center gap-1.5 rounded-full px-4 py-2.5 text-sm font-medium text-slate-800 ${glass}`}>{isJohnMaster ? "Coach John" : effectiveProfile?.display_name ?? "Coach"}<ChevronDown className={`h-3.5 w-3.5 ${roleMenuOpen ? "rotate-180" : ""}`} /></button>
            {roleMenuOpen ? <div className={`absolute right-0 z-50 mt-2 w-48 rounded-[22px] p-1.5 ${glass}`}><Link to="/" onClick={() => setRoleMenuOpen(false)} className="flex rounded-2xl px-3 py-3 text-sm font-medium">Byt till spelarvy</Link></div> : null}
          </div>
        </header>

        {message ? <p className={`mt-4 rounded-[22px] px-4 py-3 text-sm text-slate-700 ${glass}`}>{message}</p> : null}

        {view === "overview" ? <OverviewView players={visiblePlayers} blocks={visibleBlocks} onPlayer={openPlayer} onFocus={openFocus} /> : null}
        {view === "players" ? <PlayersDirectory players={visiblePlayers} blocks={visibleBlocks} onPlayer={openPlayer} /> : null}
        {view === "player-profile" && selectedPlayer ? <PlayerProfileView player={selectedPlayer} sessions={visibleSessions[selectedPlayer.relationship.player_id] ?? []} blocks={visibleBlocks} onBack={() => setView("players")} onFocus={() => openFocus(selectedPlayer.relationship.player_id)} /> : null}
...
function OverviewView({ players, blocks, onPlayer, onFocus }: { players: PlayerRow[]; blocks: FocusBlock[]; onPlayer: (id: string) => void; onFocus: (id: string) => void }) {
  const attention = players.map((player) => {
    const active = activeBlockFor(player.relationship.player_id, blocks);
    if (!active) return { player, text: "Saknar aktivt fokus" };
    const days = daysUntil(active.end_date);
    if (days <= 7) return { player, text: days < 0 ? "Fokusperioden har löpt ut" : `${days} dagar kvar av fokusperioden` };
    return null;
  }).filter(Boolean) as { player: PlayerRow; text: string }[];
  return <>
    <section className={`mt-6 rounded-[34px] p-6 ${glass}`}><span className="rounded-full border border-sky-200 bg-sky-50 px-3 py-1 text-[10px] font-semibold uppercase tracking-[.16em] text-sky-900">Översikt</span><h1 className="mt-5 text-[38px] font-medium leading-[.98] tracking-[-.055em]">Dina spelare.</h1><p className="mt-3 text-sm leading-6 text-slate-600">Håll koll på riktningen utan att detaljstyra elevens träning.</p><div className="mt-6 grid grid-cols-3 gap-2"><Metric label="Elever" value={String(players.length)} /><Metric label="Aktiva fokus" value={String(players.filter((p) => activeBlockFor(p.relationship.player_id, blocks)).length)} /><Metric label="Block" value={String(blocks.length)} /></div></section>
    <section className="mt-8"><SectionTitle eyebrow="Att följa upp" title="Fokusperioder" right={`${attention.length}`} /><div className="space-y-2.5">{attention.length ? attention.map(({ player, text }) => <button key={player.relationship.id} onClick={() => onFocus(player.relationship.player_id)} className={`flex w-full items-center justify-between rounded-[24px] p-4 text-left ${glass}`}><span><span className="block text-sm font-medium">{player.name}</span><span className="mt-1 block text-xs text-slate-600">{text}</span></span><ChevronRight className="h-4 w-4 text-slate-500" /></button>) : <div className={`rounded-[24px] p-4 text-sm text-slate-600 ${glass}`}>Alla fokusperioder ser aktuella ut.</div>}</div></section>
    <section className="mt-8"><button onClick={() => players[0] && onPlayer(players[0].relationship.player_id)} className={`flex w-full items-center justify-between rounded-[26px] p-5 text-left ${glass}`}><div><p className="text-sm font-medium">Öppna elevregistret</p><p className="mt-1 text-xs text-slate-600">Sök, filtrera och öppna elevprofiler.</p></div><Users className="h-5 w-5 text-sky-800" /></button></section>
  </>;
}
...
function PlayersDirectory({ players, blocks, onPlayer }: { players: PlayerRow[]; blocks: FocusBlock[]; onPlayer: (id: string) => void }) {
  ...
  const [club, setClub] = useState("Alla");
  const [focusStatus, setFocusStatus] = useState("Alla");
  ...
  const filtered = players.filter((player) => {
    ...
    const matchesQuery = !q || player.name.toLowerCase().includes(q) || (player.clubName ?? "").toLowerCase().includes(q);
    const matchesGender = gender === "Alla" || (player.gender ?? "Ej angivet") === gender;
    const matchesClub = club === "Alla" || (player.clubName ?? "Ej angivet") === club;
    const hasFocus = Boolean(activeBlockFor(player.relationship.player_id, blocks));
    const matchesFocus = focusStatus === "Alla" || (focusStatus === "Aktivt fokus" ? hasFocus : !hasFocus);
    return matchesQuery && matchesGender && matchesClub && matchesFocus;
  });
  return <section className="mt-7">
    <SectionTitle eyebrow="Elever" title="Alla elever" right={`${filtered.length}/${players.length}`} />
    <div className={`rounded-[28px] p-4 ${glass}`}>
      <div className="flex items-center gap-2 rounded-[18px] border border-slate-300 bg-white px-3"><Search className="h-4 w-4 text-slate-500" /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Sök elev eller klubb" className="w-full bg-transparent py-3 text-sm outline-none" /></div>
      <div className="mt-3 flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[.14em] text-slate-500"><SlidersHorizontal className="h-3.5 w-3.5" />Filter</div>
      <div className="mt-2 grid grid-cols-3 gap-2">
        <FilterSelect value={gender} onChange={setGender} options={["Alla", ...genders, ...(genders.includes("Ej angivet") ? [] : ["Ej angivet"])]} label="Kön" />
        <FilterSelect value={club} onChange={setClub} options={["Alla", ...clubs, ...(clubs.includes("Ej angivet") ? [] : ["Ej angivet"])]} label="Klubb" />
        <FilterSelect value={focusStatus} onChange={setFocusStatus} options={["Alla", "Aktivt fokus", "Utan fokus"]} label="Fokus" />
      </div>
    </div>
    <div className="mt-4 space-y-2.5">{filtered.map((player) => <DirectoryPlayerCard key={player.relationship.id} player={player} block={activeBlockFor(player.relationship.player_id, blocks)} onClick={() => onPlayer(player.relationship.player_id)} />)}{!filtered.length ? <div className={`rounded-[24px] p-5 text-center text-sm text-slate-600 ${glass}`}>Inga elever matchar filtret.</div> : null}</div>
  </section>;
}

function PlayerProfileView({ player, sessions, blocks, onBack, onFocus }: { player: PlayerRow; sessions: TrainingSession[]; blocks: FocusBlock[]; onBack: () => void; onFocus: () => void }) {
  const playerBlocks = blocks.filter((b) => b.player_id === player.relationship.player_id).sort((a, b) => b.start_date.localeCompare(a.start_date));
  const active = playerBlocks.find((b) => b.status === "active") ?? null;
  const progress = getProgress(player, sessions);
  const completedCount = playerBlocks.filter((b) => b.status !== "active").length;
  return <>
    <BackButton onClick={onBack}>Alla elever</BackButton>
    <section className={`mt-4 rounded-[32px] p-5 ${glass}`}>
      <div className="flex items-center gap-4"><span className="flex h-14 w-14 items-center justify-center rounded-full bg-slate-100 text-lg font-semibold text-slate-800 ring-1 ring-slate-300">{player.name.slice(0,1).toUpperCase()}</span><div><h1 className="text-2xl font-semibold tracking-[-.04em]">{player.name}</h1><p className="mt-1 text-sm text-slate-600">{player.clubName ?? "Klubb ej angiven"}{player.gender ? ` · ${player.gender}` : ""}</p></div></div>
      <div className="mt-5 grid grid-cols-3 gap-2"><Metric label="Start HCP" value={progress.start !== null ? formatNumber(progress.start) : "–"} /><Metric label="Nu" value={progress.current !== null ? formatNumber(progress.current) : "–"} /><Metric label="Förändring" value={progress.improvement !== null ? `${progress.improvement > 0 ? "−" : "+"}${formatNumber(Math.abs(progress.improvement))}` : "–"} positive={Boolean(progress.improvement && progress.improvement > 0)} /></div>
      <div className="mt-5 grid grid-cols-2 gap-2"><Metric label="Tillsammans" value={relationshipAge(player.relationship)} /><Metric label="Fokusblock" value={String(playerBlocks.length)} /></div>
      <p className="mt-4 text-xs text-slate-600">{relationshipText(player.relationship)} · {completedCount} avslutade/förlängda fokusperioder</p>
    </section>

    <section className="mt-8"><SectionTitle eyebrow="Från tränaren" title="Aktuellt fokus" />{active ? <FocusBlockCard block={active} /> : <div className={`rounded-[28px] p-5 text-sm text-slate-600 ${glass}`}>Inget aktivt fokus just nu.</div>}<button onClick={onFocus} className="mt-3 w-full rounded-[20px] bg-sky-700 py-3.5 text-sm font-medium text-white">{active ? "Hantera fokus" : "Skapa fokus"}</button></section>

    <section className="mt-8"><SectionTitle eyebrow="Historik" title="Fokusblock" right={`${playerBlocks.length}`} /><div className="space-y-3">{playerBlocks.map((block) => <FocusHistoryRow key={block.id} block={block} />)}{!playerBlocks.length ? <div className={`rounded-[24px] p-4 text-sm text-slate-500 ${glass}`}>Ingen fokushistorik ännu.</div> : null}</div></section>

    <section className="mt-8"><SectionTitle eyebrow="Utveckling" title="Träningsunderlag" /><div className={`rounded-[28px] p-5 ${glass}`}><p className="text-sm leading-6 text-slate-600">Data används som underlag för coachens bedömning, inte för detaljstyrning.</p><div className="mt-4 space-y-3">{sessions.slice(0,3).map((s) => <SessionSummary key={s.id} session={s} />)}{!sessions.length ? <p className="text-sm text-slate-500">Ingen träningsdata ännu.</p> : null}</div></div></section>
  </>;
}
...
function FocusEditor({ player, blocks, onBack, onCreate, onFinish, onExtend }: { player: PlayerRow; blocks: FocusBlock[]; onBack: () => void; onCreate: (input: Omit<FocusBlock, "id" | "relationship_id" | "player_id" | "coach_id" | "status" | "extended_from" | "created_at">) => void; onFinish: (block: FocusBlock) => void; onExtend: (block: FocusBlock) => void }) {
  const active = activeBlockFor(player.relationship.player_id, blocks);
  const [primary, setPrimary] = useState(""); const [secondary, setSecondary] = useState(""); const [why, setWhy] = useState(""); const [note, setNote] = useState(""); const [recommendations, setRecommendations] = useState(""); const [weeks, setWeeks] = useState(6);
  ...
  return <>
    <BackButton onClick={onBack}>{player.name}</BackButton>
    {active ? <section className="mt-4"><SectionTitle eyebrow="Aktivt" title="Nuvarande fokus" /><FocusBlockCard block={active} /><div className="mt-3 grid grid-cols-2 gap-2"><button onClick={() => onExtend(active)} className="rounded-[18px] border border-sky-300 bg-sky-50 py-3 text-sm font-medium text-sky-900">Förläng fokus</button><button onClick={() => onFinish(active)} className="rounded-[18px] border border-slate-300 bg-white py-3 text-sm font-medium text-slate-700">Avsluta</button></div></section> : null}
    <section className="mt-8"><SectionTitle eyebrow={active ? "Nästa riktning" : "Nytt fokus"} title="Fokusperiod" /><div className={`rounded-[30px] p-5 ${glass}`}><FieldLabel>Huvudfokus</FieldLabel><input value={primary} onChange={(e) => setPrimary(e.target.value)} placeholder="Ex. Around the Green" className="mt-2 w-full rounded-[18px] border border-slate-300 bg-white px-4 py-3 text-sm outline-none" /><FieldLabel className="mt-4">Sekundärt fokus</FieldLabel><input value={secondary} onChange={(e) => setSecondary(e.target.value)} placeholder="Valfritt" className="mt-2 w-full rounded-[18px] border border-slate-300 bg-white px-4 py-3 text-sm outline-none" /><FieldLabel className="mt-4">Riktning</FieldLabel><textarea value={why} onChange={(e) => setWhy(e.target.value)} rows={2} placeholder="Vad bör eleven prioritera?" className="mt-2 w-full resize-none rounded-[18px] border border-slate-300 bg-white px-4 py-3 text-sm outline-none" /><FieldLabel className="mt-4">Coachens kommentar</FieldLabel><textarea value={note} onChange={(e) => setNote(e.target.value)} rows={2} placeholder="Ex. Tänk på händerna framför bollen när du chippar." className="mt-2 w-full resize-none rounded-[18px] border border-slate-300 bg-white px-4 py-3 text-sm outline-none" /><FieldLabel className="mt-4">Rekommenderade tester & övningar</FieldLabel><textarea value={recommendations} onChange={(e) => setRecommendations(e.target.value)} rows={3} placeholder={"En per rad\n8-bollar\nUpp & in"} className="mt-2 w-full resize-none rounded-[18px] border border-slate-300 bg-white px-4 py-3 text-sm outline-none" /><FieldLabel className="mt-4">Period</FieldLabel><div className="mt-2 grid grid-cols-4 gap-2">{[2,4,6,8].map((w) => <button key={w} onClick={() => setWeeks(w)} className={`rounded-[16px] border py-2.5 text-xs font-medium ${weeks === w ? "border-sky-400 bg-sky-100 text-sky-900" : "border-slate-300 bg-white text-slate-600"}`}>{w} v</button>)}</div><button onClick={save} disabled={!primary.trim()} className="mt-5 w-full rounded-[20px] bg-sky-700 py-3.5 text-sm font-medium text-white disabled:opacity-40">{active ? "Starta nytt fokus" : "Skapa fokus"}</button></div></section>
  </>;
}
...
function CoachNav({ view, onChange }: { view: CoachView; onChange: (view: CoachView) => void }) {
  const items = [["overview", "Översikt", LayoutDashboard], ["players", "Elever", Users], ["progress", "Utveckling", BarChart3], ["more", "Mer", MoreHorizontal]] as const;
  return <nav className="fixed inset-x-0 bottom-0 z-50 mx-auto w-full max-w-md px-4 pb-[max(12px,env(safe-area-inset-bottom))]"><div className="grid grid-cols-4 rounded-[30px] border border-slate-200 bg-white/98 px-2 py-2 shadow-[0_18px_48px_-16px_rgba(30,41,59,.40)] backdrop-blur-2xl">{items.map(([key,label,Icon]) => <button key={key} onClick={() => { onChange(key); window.scrollTo({top:0,behavior:"smooth"}); }} className={`flex flex-col items-center gap-1 rounded-[20px] px-1 py-2 ${view === key ? "bg-sky-100 text-sky-900" : "text-slate-500"}`}><Icon className="h-[19px] w-[19px]" /><span className="text-[9px] font-semibold">{label}</span></button>)}</div></nav>;
}
