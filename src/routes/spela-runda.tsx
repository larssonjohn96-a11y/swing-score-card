...
function RoundGamesPage() {
  ...
  const Card=({game,small=false}:{game:(typeof GAMES)[number];small?:boolean})=><div className={`relative overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-sm ${small?"h-[190px] w-[164px] shrink-0":"h-[220px] w-full"}`}>
    ...
    <button type="button" aria-label={favorites.includes(game.to)?"Ta bort favorit":"Lägg till favorit"} onClick={()=>toggle(game.to)} className="absolute right-3 top-3 z-20 flex h-8 w-8 items-center justify-center rounded-full bg-white/90 shadow-sm"><Star className={`h-4 w-4 ${favorites.includes(game.to)?"fill-amber-400 text-amber-400":"text-slate-500"}`}/></button>
  </div>;
  return (
    <main className="mx-auto min-h-screen max-w-md bg-slate-50 px-5 pb-8 pt-[max(16px,env(safe-area-inset-top))] text-slate-950">
      ...
      <section className="mt-8">
        ...
        <div className="-mx-5 mt-3 flex gap-2 overflow-x-auto px-5 pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{GAMES.filter(g=>g.to==="/speedrundan").map(game=><Card key={game.to} game={game} small />)}</div>
      </section>
      <section className="mt-8">
        <h2 className="px-0.5 font-display text-[28px] leading-none text-slate-950">Off the Tee</h2>
        <div className="-mx-5 mt-3 flex gap-2 overflow-x-auto px-5 pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{GAMES.filter(g=>g.to==="/longdrive"||g.to==="/driverrundan").map(game=><Card key={game.to} game={game} small />)}</div>
      </section>
      <section className="mt-8">
        <h2 className="px-0.5 font-display text-[28px] leading-none text-slate-950">Inspel</h2>
        <div className="-mx-5 mt-3 flex gap-2 overflow-x-auto px-5 pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{GAMES.filter(g=>g.to==="/inspelsrundan").map(game=><Card key={game.to} game={game} small />)}</div>
      </section>
      <section className="mt-8">
        <h2 className="px-0.5 font-display text-[28px] leading-none text-slate-950">Puttning</h2>
        <div className="-mx-5 mt-3 flex gap-2 overflow-x-auto px-5 pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{GAMES.filter(g=>g.to==="/puttrundan").map(game=><Card key={game.to} game={game} small />)}</div>
      </section>
      <section className="mt-8">
