...
export const Route = createFileRoute("/tester")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "HCP Test – SG4" },
...
function HcpCard({ category, test, index }: { category: Category; test: CategoryTest; index: number }) {
    ...
    <Link to={test.to} className="block w-[164px] shrink-0">
      <article className={`relative flex h-[220px] flex-col justify-end overflow-hidden rounded-[24px] border border-black/[.04] px-4 pb-4 pt-4 text-white ${toneFor(category, index)}`}>
        ...
        <div className="relative z-10">
          ...
          <p className="mt-2 line-clamp-2 text-[11px] leading-snug text-white/72">{test.subtitle}</p>
        </div>
      </article>
    </Link>
  );
      ...
      <header className="sticky top-0 z-30 border-b border-border/70 bg-background/90 px-5 pb-4 pt-[max(14px,env(safe-area-inset-top))] backdrop-blur-2xl">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <h1 className="mt-0.5 text-[26px] font-black leading-none text-foreground">HCP Test</h1>
          </div>
        </div>
        ...
        {CATEGORIES.map((category) => (
          <section key={category.slug}>
            ...
            <div className="-mx-5 mt-3.5 flex gap-2 overflow-x-auto px-5 pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              {category.tests.map((test, index) => <HcpCard key={`${category.slug}-${test.title}`} category={category} test={test} index={index} />)}
            </div>
          </section>
        ))}
      </div>
